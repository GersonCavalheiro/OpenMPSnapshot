...
{
*p_ele_node_local_idx_pair = nodes_ele_node_local_idx_pair[m][n * 2 + 0]; p_ele_node_local_idx_pair++; 
*p_ele_node_local_idx_pair = nodes_ele_node_local_idx_pair[m][n * 2 + 1]; p_ele_node_local_idx_pair++; 
}
}
}
};
...
{
public:
vector<float> m_external_F,          m_ele_nodal_internal_F,                        
m_disp_mag_t,
m_central_diff_const1, m_central_diff_const2, m_central_diff_const3,
m_prev_U,              m_curr_U,              m_next_U,
m_external_Q,          m_external_Q0,         m_ele_nodal_internal_Q, 
m_fixT_mag,
m_constA,
m_curr_T,              m_next_T;
...
fscanf_s(file, "%u %f %f %f", &idx, &x, &y, &z); model->m_node_begin_index = idx; model->m_nodes.push_back(new Node(idx - model->m_node_begin_index, x, y, z)); 
while (fscanf_s(file, "%u %f %f %f", &idx, &x, &y, &z)) { model->m_nodes.push_back(new Node(idx - model->m_node_begin_index, x, y, z)); } 
...
float mag = sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]); if (mag != 1.f) { a[0] /= mag; a[1] /= mag; a[2] /= mag; } 
...
else if (model->m_M_material_type == "other_material_types") {  }
...
else if (model->m_T_material_type == "other_conductivity_types") {  }
...
const float mag = sqrt(m[0] * m[0] + m[1] * m[1] + m[2] * m[2]); if (mag != 1.f) { m[0] /= mag; m[1] /= mag; m[2] /= mag; } 
...
const float magm = sqrt(m[0] * m[0] + m[1] * m[1] + m[2] * m[2]); if (magm != 1.f) { m[0] /= magm; m[1] /= magm; m[2] /= magm; } 
const float M00 = m[0] * m[0], M01 = m[0] * m[1], M02 = m[0] * m[2], M11 = m[1] * m[1], M12 = m[1] * m[2], M22 = m[2] * m[2];
model->m_T_expan_vals.push_back(alpha_m - alpha_i); model->m_T_expan_vals.push_back(M00); model->m_T_expan_vals.push_back(M01); model->m_T_expan_vals.push_back(M02); model->m_T_expan_vals.push_back(M11); model->m_T_expan_vals.push_back(M12); model->m_T_expan_vals.push_back(M22);
const float magn = sqrt(n[0] * n[0] + n[1] * n[1] + n[2] * n[2]); if (magn != 1.f) { n[0] /= magn; n[1] /= magn; n[2] /= magn; } 
...
else if (model->m_T_expan_type == "other_expansion_types") {  }
...
model->m_rho, model->m_M_material_type, model->m_M_material_vals, model->m_T_material_type, model->m_T_material_vals, model->m_T_expan_type, model->m_T_expan_vals)); 
...
else if (BC_type == "other_BC_types") {  }
...
cout << "\tcomputing..." << endl;
for (size_t step = 0; step < model.m_num_steps; step++) 
{
...
for (int i = id; i < model.m_tets.size(); i += NUM_THREADS) 
{
...
else if (tet->m_T_expan_type == "other_expansion_types") {  }
matInv33(tet->m_X_expan, invX_expan, J_invX_expan);
mat33x33(tet->m_X, invX_expan, temp_X); 
}
mat33Tx33(temp_X, temp_X, C); 
...
const2(-const1 * I1 / 3.f + tet->m_M_material_vals[1] * J * (J - 1.f)); 
...
const4(-(const1 * I1 + 2.f * const2 * I4cap) / 3.f + tet->m_M_material_vals[1] * J * (J - 1.f)); 
...
else if (tet->m_M_material_type == "other_material_types") {  }
if (tet->m_T_expan_vals.size() != 0)
{
...
#pragma omp barrier
for (int i = id; i < model.m_nodes.size(); i += NUM_THREADS) 
{
...
{
n_DOF = i * 3 + j;
if (modelstates.m_disp_mag_t[n_DOF] != 0.f) { modelstates.m_next_U[n_DOF] = modelstates.m_disp_mag_t[n_DOF]; } 
else if (modelstates.m_fixP_flag[n_DOF] == true) { modelstates.m_next_U[n_DOF] = 0.f; }                        
else                                                                                                           
{
...
if (modelstates.m_fixT_flag[i] == true) { modelstates.m_next_T[i] = modelstates.m_fixT_mag[i]; } 
else                                                                                             
{
...
int exportVTK(const Model& model, const ModelStates& modelstates)
{
const vector<string> outputs{ "U.vtk", "Undeformed.vtk", "T.vtk" }; 
cout << "\n\texporting..." << endl;

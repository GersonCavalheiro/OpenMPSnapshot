...
{
#pragma omp atomic
optimization[i]->derivative += enter[i] * d_res_function_multiplied_error; 
}
...
transform(enter.cbegin(), enter.cend(), w.cbegin(), for_sum.begin(), [](const double& a, const double& b) {return a * b; }); 
for_sum.back() = -w.back(); 

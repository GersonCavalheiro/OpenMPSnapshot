...
_gemm_in_avx(cm1, cm2, temp_avx_result); 
for (int pi = 0; pi < sample_tile_size; pi++) {
for (int pj = 0; pj < sample_tile_size; pj++) {
temp_result[pi * sample_tile_size + pj] += temp_avx_result[pi * sample_tile_size + pj]; 
}
}
}
for (int ci = 0; ci < sample_tile_size; ci++) {
for (int cj = 0; cj < c.M2_samples; cj++) {
result[(i * sample_tile_size + ci) * c.M2_samples + cj] = temp_result[ci * sample_tile_size + cj]; 
}
}
}
free(cm1);
...
{
for (int i = 0; i < NORM_SAMPLE_TILE_SIZE; i += 1) { 
...
__m256 xx_vec;
for (int i = 0; i < FUSE_T1_SIZE; i += 1) { 

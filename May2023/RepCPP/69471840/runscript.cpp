...
application[ TExternalApp::USER_COMMAND ]          = wxString( wxT("") ); 
...
int appNumber = static_cast<int>( TExternalApp::DIMEMAS_WRAPPER ); 
...
parameters += wxString( wxT( " " ) ) + doubleQuote( fileBrowserButtonDimemasCFG->GetPath() ); 
parameters += wxString( wxT( " " ) ) + doubleQuote( textCtrlOutputTrace->GetValue() ); 
...
if ( !radioButtonDimemasDefaultTasksMapping->GetValue() ) 
{
...
parameters += wxString( wxT( " " ) ) + expandVariables( textCtrlDefaultParameters->GetValue() ); 
...
parameters = doubleQuote( fileBrowserButtonTrace->GetPath() ); 
parameters += wxString( wxT( " -o " ) ) + doubleQuote( statsTextCtrlOutputName->GetValue() ); 
...
expandVariables( textCtrlDefaultParameters->GetValue() ); 
}
...
{
parameters += doubleQuote( expandVariables( textCtrlDefaultParameters->GetValue() ) ); 
}
...
if ( currentApp == TExternalApp::CLUSTERING && matchHrefExtension( event, extensions[ 10 ] ) ) 
{
wxString tmpSuffixToErase = extensions[ 10 ]; 
...
filter->setEventTypeFunction( std::string( "=" ) ); 
...
else if ( matchHrefExtension( event, extensions[8] )) 
{
bool found;
...
else if ( matchHrefExtension( event, extensions[9] ))  
{
bool found;
gTimeline *currentWindow = getGTimelineFromWindow( getAllTracesTree()->GetRootItem(), paraverMain::myParaverMain->GetCurrentTimeline(), found );
if ( found )

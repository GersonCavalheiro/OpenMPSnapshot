...
void write(NetworkBitStream& bs) const
{
bs.writeBIT(EnableZoneNames); 
bs.writeBIT(UsePlayerPedAnims); 
bs.writeBIT(AllowInteriorWeapons); 
bs.writeBIT(UseLimitGlobalChatRadius); 
bs.writeFLOAT(LimitGlobalChatRadius); 
bs.writeBIT(EnableStuntBonus); 
bs.writeFLOAT(SetNameTagDrawDistance); 
bs.writeBIT(DisableInteriorEnterExits); 
bs.writeBIT(DisableNameTagLOS); 
bs.writeBIT(ManualVehicleEngineAndLights); 
bs.writeUINT32(SetSpawnInfoCount); 
bs.writeUINT16(PlayerID); 
bs.writeBIT(ShowNameTags); 
bs.writeUINT32(ShowPlayerMarkers); 
bs.writeUINT8(SetWorldTime); 
bs.writeUINT8(SetWeather); 
bs.writeFLOAT(SetGravity); 
bs.writeBIT(LanMode); 
bs.writeUINT32(SetDeathDropAmount); 
bs.writeBIT(Instagib); 
bs.writeUINT32(OnFootRate); 
bs.writeUINT32(InCarRate); 
bs.writeUINT32(WeaponRate); 
bs.writeUINT32(Multiplier); 
bs.writeUINT32(LagCompensation); 

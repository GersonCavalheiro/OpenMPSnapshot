...
#define TRANS_MEM_SIZE (1 * 1024 * 1024 * 1024)  
...
const index_type pivot_i = pivot[(numOfCDVcts + 1) + c];  
...
std::vector<double> global_diagonals;  
...
std::vector<int> reverse_pivot(N);  
...
std::vector<index_type> G_col_list(numOf_G_cols);
for (index_type i = 0; i < numOf_G_cols; ++i) {
const index_type pivot_i = global_pivot[m + 1 + i];  
...
this->log_.info("load density matrix");
const TlDenseSymmetricMatrix_Scalapack P =
0.5 * DfObject::getPpqMatrix<TlDenseSymmetricMatrix_Scalapack>(runType, this->m_nIteration - 1);  

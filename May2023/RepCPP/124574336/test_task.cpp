...
void operator()() const {
parallel_for_body body(isolated_regular_task,  true);
...
void operator()() const {
while( !my_root_task ) __TBB_Yield(); 
...
g_arena.execute(
WorkCreator<TaskBody, task::spawn>(g_root_task, size_t(num_threads)) );
...
ATask( task_marker_t  ) : my_cancellation_method( no_cancellation ) {}
...
cancelled_group                             
&& !this->is_cancelled()                    
&& !tbb::internal::is_critical(*this)       
&& bypassed == previous_critical_task_stage 
) __TBB_Yield();
if( cancelled_group )
...
g_arena.initialize( MaxThread ); 
g_arena.execute(
WorkCreator<ATask<true>, task::spawn>
(root_task_of_to_be_cancelled_context, task_num_for_cancelled_context,
0, &context_to_cancel)
);
g_arena.execute(
WorkCreator<ATask<false>, task::spawn>
(root_task_of_working_to_completion_context, task_num, 1,
...
ATask<true>( cancellation_method );
tbb::internal::make_critical( terminating_task ); 
...
ASSERT( g_num_executed_from_working_context == task_num + 1,
"Incorrect number of tasks executed!" );
...
g_is_critical_task_submitted = true;
g_arena.execute(
WorkCreator<CriticalTask, task::spawn>(nested_root, size_t(0),
...
g_root_task, size_t(0), g_num_critical_tasks) );
...
TestCancellationSupport(false);
TestCancellationSupport(true);

...
new_buffer,
new_capacity );
copy_impl( begin(), end(), new_buffer ); 
guard.dismiss();                         
return new_buffer;
}
...
void one_sided_swap( auto_buffer& temp ) 
{
...
auto_buffer& operator=( const auto_buffer& r ) 
{
if( this == &r )
return *this;
...
void unchecked_push_back( optimized_const_reference x ) 
{
BOOST_ASSERT( !full() );
...
iterator insert( const_iterator before, optimized_const_reference x ) 
{
if( size_ < members_.capacity_ )
{
bool is_back_insertion = before == cend();
...
ForwardIterator begin_arg, ForwardIterator end_arg ) 
{
...
pointer uninitialized_grow( size_type n ) 
{
if( size_ + n > members_.capacity_ )
reserve( size_ + n );
...
new_buffer );                            
one_on_stack->auto_buffer_destroy();                       
boost::swap( get_allocator(), r.get_allocator() );  
boost::swap( members_.capacity_, r.members_.capacity_ );
boost::swap( size_, r.size_ );

...
bool open(std::string const &file_name,std::string const &)
{
close();
...
st = pj_winberger_hash::update_state(st,'\4'); 
...
static pair_type use(mo_file const &,char_type const * ,char_type const * )
{
...
class converter {
public:
converter(std::string ,std::string in_enc) :
in_(in_enc)
{
}

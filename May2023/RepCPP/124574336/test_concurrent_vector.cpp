...
vector_t copy_of_v(v); 
if(__TBB_Log2(old_size|1) > __TBB_Log2(fill_size|1) )
ASSERT( v.capacity() != copy_of_v.capacity(), NULL );
v.shrink_to_fit();
ASSERT( v.capacity() == copy_of_v.capacity(), NULL );
CheckVector(v, new_size*2, old_size); 
ASSERT( v==copy_of_v, NULL ); 
}
...
static const size_t number_of_grains = 8; 
...
var[2].assign(var[1].rbegin(), var[1].rend()); 
for (int i = 0; i < 3; ++i) {
for (int j = 0; j < 3; ++j) {
...
{
if (is_memory_allocation_failure) { 
...
try {
vector_t src(FooIterator(0), FooIterator(N)); 
...
case compact: {
if(t) MaxFooCount = 0; else allocator_t::set_limits(); 
victim.reserve(2); victim = src; 
if(t) MaxFooCount = FooCount + 10; else allocator_t::set_limits(1, false); 
...
ASSERT_THROWS_IN_TEST(
{
limit_foo_count_in_scope foo_limit(FooCount +  31, t); 
limit_allocated_items_in_scope<allocator_t> allocator_limit(allocator_t::items_allocated + planned_victim_size, !t);
...
victim = std::move(fixture.source); 
},
std::bad_alloc, "", test_name
);

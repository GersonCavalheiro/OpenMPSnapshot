...
namespace  { namespace hyperexp_detail {
...
{
return std::make_pair(static_cast<RealT>(0), std::numeric_limits<RealT>::infinity()); 
}

return std::make_pair(static_cast<RealT>(0), tools::max_value<RealT>()); 
}

template <typename RealT, typename PolicyT>
std::pair<RealT,RealT> support(hyperexponential_distribution<RealT,PolicyT> const&)
{
return std::make_pair(tools::min_value<RealT>(), tools::max_value<RealT>()); 
}

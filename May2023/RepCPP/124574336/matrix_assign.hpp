...
if (static_cast<value_type>(*it2e) != value_type())
...
if (static_cast<value_type>(*it1e) != value_type())
...
while (-- size2 >= 0)
functor_type::apply (*it2, expr_value_type()), ++ it2;
++ it1;
}
} else {
it1 += size1;
...
while (-- size1 >= 0)
functor_type::apply (*it1, expr_value_type()), ++ it1;
++ it2;
}
} else {
it2 += size2;
...
#pragma warning(pop)
#endif
functor_type::apply (*it2, expr_value_type());
++ it2;
} else
increment (it2, it2_end, - compare2);
if (it2 != it2_end)
...
#pragma warning(pop)
#endif
functor_type::apply (*it1, expr_value_type()); 
++ it1;
} else
increment (it1, it1_end, - compare2);
if (it1 != it1_end)

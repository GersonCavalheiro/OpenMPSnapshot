...
private:
template< class I >
void insert_impl( iterator before, I first, I last, std::input_iterator_tag ) 
{
ptr_sequence_adapter temp(first,last);  
transfer( before, temp );               
}

template< class I >
void insert_impl( iterator before, I first, I last, std::forward_iterator_tag ) 
{
if( first == last ) 
return;
scoped_deleter sd( *this, first, last );         
this->insert_clones_and_release( sd, before );   
}
...
return;
this->base().
insert( before.base(), first.base(), last.base() ); 
from.base().erase( first.base(), last.base() );         
}
...
if( from.empty() )
return;
this->base().insert( before.base(), *object.base() ); 
from.base().erase( object.base() );                  
}
...
deleter( *this, from, size );                         
this->base().insert( before.base(), from, from + size );  
deleter.release();                                        
}
else
{
...
private:
void compact_and_erase_nulls( iterator first, iterator last ) 
{

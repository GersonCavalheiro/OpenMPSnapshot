...
BOOST_CONTAINER_FORCEINLINE void flat_tree_container_inplace_merge 
...
template<class SequenceContainer, class Compare>
BOOST_CONTAINER_FORCEINLINE void flat_tree_container_inplace_sort_ending 
(SequenceContainer& dest, typename SequenceContainer::iterator it, Compare comp, dtl::true_)
{
typedef typename SequenceContainer::value_type  value_type;
value_type *const iraw = boost::movelib::iterator_to_raw_pointer(it);
...
template<class SequenceContainer, class Compare>
BOOST_CONTAINER_FORCEINLINE void flat_tree_adopt_sequence_equal 
...
typedef container_type sequence_type;  
...
if(pos == cend_it || key_cmp(k, KeyOfValue()(*pos))){ 
...
--prev;
if(key_cmp(KeyOfValue()(*prev), k)){   
return true;
}
else if(!key_cmp(k, KeyOfValue()(*prev))){   

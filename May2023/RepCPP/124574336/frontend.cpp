...
memPool->returnEmptyBlock(block, false);
released = true;
}
}
return released;
...
{
bool released = allLocalCaches.cleanup(false);
...
released |= static_cast<TLSData*>(curr)->externalCleanup(cleanOnlyUnused, false);
}
return released;
}
...
block->reset();
if (poolTheBlock) {
getTLS(false)->freeSlabBlocks.returnBlock(block);
} else {
if (!extMemPool.userPool())
...
void Block::adjustPositionInBin(Bin* bin)
{
...
tlsPtr->getAllocationBin(objectSize)->processEmptyBlock(this, true);
} else { 
...
block->privatizePublicFreeList( false ); 
if (block->empty()) {
block->reset();
...
memPool->returnEmptyBlock(threadlessBlock, false);
} else {
...
#endif
{
suppress_unused_warning(tls); 
defaultMemPool->onThreadShutdown(defaultMemPool->getTLS(false));
bool locked;
MallocMutex::scoped_lock lock(MemoryPool::memPoolListLock, !main_thread, &locked);
if (locked) { 
for (MemoryPool *memPool = defaultMemPool->next; memPool; memPool = memPool->next)
memPool->onThreadShutdown(memPool->getTLS(false));
}
}
...
doThreadShutdownNotification(NULL, true);
}
#if  __TBB_MALLOC_LOCACHE_STAT
...
released = tls->externalCleanup(false, true);
break;

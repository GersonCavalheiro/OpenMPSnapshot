...
{
eraser e(&this->base(),key);      
mapped_type res = new T();        
ref = res;                        
e.release();                      
return *res;
}
...
auto_type old( where->second, *this ); 
where.base()->second = ptr.release();  
...
void safe_insert( const key_type& key, auto_type ptr ) 
{
std::pair<BOOST_DEDUCED_TYPENAME base_type::ptr_iterator,bool>
res = 
this->base().insert( std::make_pair( key, ptr.get() ) ); 
if( res.second )                                             
ptr.release();                                           
}
...
private:
std::pair<iterator,bool> insert_impl( const key_type& key, mapped_type x ) 
{
...
iterator insert_impl( iterator before, const key_type& key, mapped_type x ) 
{
...
#ifndef BOOST_NO_CXX11_SMART_PTR
template< class U >
iterator insert( iterator before, const key_type& key, std::unique_ptr<U> x ) 
{
return insert_impl( before, key, x.release() );
}
#endif
...
while( first != last )                                            
{                                            
const_reference pair = *first.base();     
...
private:
iterator insert_impl( const key_type& key, mapped_type x ) 
{
this->enforce_null_policy( x, 

...
template< class I >
void constructor_impl( I first, I last, std::input_iterator_tag ) 
{
while( first != last )
{
...
template< class I >
void constructor_impl( I first, I last, std::forward_iterator_tag ) 
{
if( first == last )
return;
...
template< class I >
void associative_constructor_impl( I first, I last ) 
{
if( first == last )
return;
...
#ifndef BOOST_NO_CXX11_SMART_PTR
template< class PtrContainer >
reversible_ptr_container& operator=( std::unique_ptr<PtrContainer> clone ) 
{
swap( *clone );
return *this;
}
...
reversible_ptr_container& operator=( reversible_ptr_container r ) 
{
swap( r );
return *this;
}
...
{ 
boost::swap( get_clone_allocator(), r.get_clone_allocator() ); 
c_.swap( r.c_ ); 
}
...
bool operator==( const reversible_ptr_container& r ) const 
{ 
if( size() != r.size() )
return false;
else
...
iterator res( c_.insert( before.base(), x ) ); 
ptr.release();                                 
return res;
}
...
auto_type ptr( Config::get_pointer(where), *this );  
c_.erase( where.base() );                            
...
auto_type old( Config::get_pointer(where), *this );  
...
auto_type replace( size_type idx, Ty_* x ) 
{
enforce_null_policy( x, "Null pointer in 'replace()'" );            
...
auto_type old( static_cast<Ty_*>(c_[idx]), *this ); 
c_[idx] = ptr.release();                            
return boost::ptr_container_detail::move( old );
} 

...
if( i == this->end() )                                  
return 0u;                                          
key = static_cast<key_type*>(*i.base());                
size_type res = this->base().erase( key );              
this->remove( key );                                    
return res;
}
...
void set_basic_clone_and_insert( II first, II last ) 
{                                                                     
while( first != last )                                            
{           
if( this->find( *first ) == this->end() )
insert( this->null_policy_allocate_clone_from_iterator( first ) ); 
++first;                                                      
}                                                                 
}                         

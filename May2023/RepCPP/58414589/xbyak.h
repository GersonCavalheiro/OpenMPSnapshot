...
#if (not +0) && !defined(XBYAK_NO_OP_NAMES) 
...
void setBit(int bit);
void setOpmaskIdx(int idx, bool  = true)
{
...
{
return (kind == 0 || (kind_ & kind)) && (bit == 0 || (bit_ & bit)); 
}
...
void *const DontSetProtectRWE = (void*)2; 
...
bool isOnlyDisp() const { return !e_.getBase().getBit() && !e_.getIndex().getBit(); } 
...
bool is64bitDisp() const { return mode_ == M_64bitDisp; } 
...
if (op.isBit(32|64) && immBit == 16) immBit = 32; 
...
const AddressFrame ptr, byte, word, dword, qword, xword, yword, zword; 
const AddressFrame ptr_b, xword_b, yword_b, zword_b; 
const Fpu st0, st1, st2, st3, st4, st5, st6, st7;
const Opmask k0, k1, k2, k3, k4, k5, k6, k7;
const BoundsReg bnd0, bnd1, bnd2, bnd3;
const EvexModifierRounding T_sae, T_rn_sae, T_rd_sae, T_ru_sae, T_rz_sae; 
...
template <>
inline void CodeGenerator::mov(const NativeReg& reg, const char *label) 
{
assert(label);
mov_imm(reg, dummyAddr);
putL(label);

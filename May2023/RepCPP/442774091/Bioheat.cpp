...
{
public:
vector<float> m_external_Q, m_external_Q0, m_ele_nodal_internal_Q, 
m_fixT_mag,
m_constA,
m_curr_T,     m_next_T;
vector<bool>  m_fixT_flag;
ModelStates(const Model& model) :
m_external_Q(model.m_num_DOFs,        0.f), m_external_Q0(model.m_num_DOFs,        0.f), m_ele_nodal_internal_Q(model.m_tets.size() * 4, 0.f), 
...
else if (model->m_material_type == "other_conductivity_types") {  }
...
int exportVTK(const Model& model, const ModelStates& modelstates)
{
const vector<string> outputs{ "T.vtk" }; 
cout << "\n\texporting..." << endl;

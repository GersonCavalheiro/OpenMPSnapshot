...
tbslas::FieldSetFunctor<double, FMM_Tree_t> vel_field_functor(
vel_field_elems, vel_field_times);

if (sim_config->vtk_save_rate) {
for (int i = 1; i < 4; i++) {
vel_field_elems[i]->Write2File(
tbslas::GetVTKFileName(i - 1, "vel").c_str(), sim_config->vtk_order);
}
}
...
{
ComputeTrajRK2(vel_field_functor, dprts_points_pos, tcurr,
tcurr - TBSLAS_DT, sim_config->num_rk_step,
dprts_points_pos);
trc_evaluator(dprts_points_pos.data(), treen_num_points,
treec_points_val.data());
ComputeTrajRK2(vel_field_functor, dprts_points_pos, tcurr - TBSLAS_DT,
...
*tvel_new);
if (sim_config->vtk_save_rate &&
((timestep + 2) % sim_config->vtk_save_rate == 0)) {
tvel_new->Write2File(
tbslas::GetVTKFileName((timestep + 2), "vel").c_str(),
sim_config->vtk_order);
}

...
Real_t TBSLAS_LAPLACE_DT = 0.25;
Real_t TBSLAS_DIFF_COEFF = 0.0001;
TBSLAS_ALPHA = 3.0 / 2.0 / TBSLAS_LAPLACE_DT / TBSLAS_DIFF_COEFF;
...
TBSLAS_MOD_STOKES_DIFF_COEFF = 1;
Real_t TBSLAS_DT = 0.03125;
TBSLAS_MOD_STOKES_ALPHA = 3.0 / 2.0 / TBSLAS_DT;
const pvfmm::Kernel<double> modified_stokes_vel =
pvfmm::BuildKernel<double, tbslas::modified_stokes_vel>(
tbslas::GetModfiedStokesKernelName<double>(
TBSLAS_MOD_STOKES_ALPHA, TBSLAS_MOD_STOKES_DIFF_COEFF),
3, std::pair<int, int>(3, 3));
...
pvfmm::par::ScatterReverse(
trg_value, trg_scatter, *tree.Comm(),
tree_data.trg_coord.Dim() * mykernel->ker_dim[1] / COORD_DIM);
pvfmm::Profile::Toc();
}
pvfmm::Profile::Toc();
}
...
NULL, 10);
bool sp = (1 == strtoul(commandline_option(
argc, argv, "-sp", "0", false,

...
CheckChebOutput<FMM_Tree_t>(tconc, fn_poten_, mykernel->ker_dim[1], in_al2,
...
sim_config->tree_num_points_per_octanct, sim_config->tree_chebyshev_order,
sim_config->tree_max_depth, sim_config->tree_adap,
sim_config->tree_tolerance, comm, fn_veloc_, 3, *tvelp);
...
sim_config->tree_num_points_per_octanct, sim_config->tree_chebyshev_order,
sim_config->tree_max_depth, sim_config->tree_adap,
sim_config->tree_tolerance, comm, fn_veloc_, 3, *tvelc);
if (sim_config->vtk_save_rate) {
...
tcurr = tcurr_init;
for (int timestep = 1; timestep < sim_config->total_num_timestep + 1;
timestep += 1) {
...
dprts_points_pos);
tconc_functor(dprts_points_pos.data(), treen_num_points,
...
dprts_points_pos);
tconp_functor(dprts_points_pos.data(), treen_num_points,
...
sim_config->tree_adap, sim_config->tree_tolerance, comm, fn_veloc_, 3,
*tvel_new);
if (sim_config->vtk_save_rate &&
((timestep) % sim_config->vtk_save_rate == 0)) {
...
double al2, rl2, ali, rli;
CheckChebOutput<FMM_Tree_t>(tconc, fn_poten_, mykernel->ker_dim[1], al2, rl2,

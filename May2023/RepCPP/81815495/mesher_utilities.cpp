...
std::string DomainName = rGeometry[0].GetValue(MODEL_PART_NAME); 
...
{
std::vector<Vector> FaceCoordinates(lnofa[i]); 
...
double CriticalRelativeSideLength = (double)size * 5; 
...
BoundedMatrix<double, 2, 2> mJ;    
BoundedMatrix<double, 2, 2> mJinv; 
...
rVolume = mJ(0, 0) * mJ(1, 1) - mJ(0, 1) * mJ(1, 0); 
...
BoundedVector<double, 3> mRHS;     
BoundedMatrix<double, 3, 3> mJ;    
BoundedMatrix<double, 3, 3> mJinv; 
...
rVolume = mJ(0, 0) * mJinv(0, 0) + mJ(0, 1) * mJinv(1, 0) + mJ(0, 2) * mJinv(2, 0); 
...
if (condition_found)
{
pCondition->Set(SELECTED.AsFalse()); 
face_elements++;
break;
}
}
...
array_1d<double, 3> &Normal = (nodes_begin + i)->FastGetSolutionStepValue(NORMAL); 
double Shrink = (nodes_begin + i)->FastGetSolutionStepValue(SHRINK_FACTOR);        

...
SizeType bucket_size = mThisParameters["search_parameters"]["bucket_size"].GetInt(); 
...
std::vector<PointType::Pointer> points_array (TDim); 
...
GeometricalProjectionUtilities::FastProjectDirection( rMasterGeometry, gp_global, projected_gp_global, rMasterNormal, -gp_normal ); 
...
if (rCond.Has( INDEX_SET )) {
IndexSet::Pointer p_indexes_pairs = rCond.GetValue( INDEX_SET ); 
...
if (rElem.Has( INDEX_SET )) {
IndexSet::Pointer p_indexes_pairs = rElem.GetValue( INDEX_SET ); 
ClearIndexes<IndexSet>(p_indexes_pairs, rElem, rTLS.integration_utility);
} else {
IndexMap::Pointer p_indexes_pairs = rElem.GetValue( INDEX_MAP ); 
...
IndexSet::Pointer p_indexes_pairs = r_cond.GetValue( INDEX_SET ); 
...
if (mOptions.Is(ORIGIN_SKIN_IS_CONDITION_BASED)) {
auto& r_cond_master = mOriginModelPart.GetCondition(master_id); 
(r_cond_master.GetValue(INDEX_SET))->AddId(r_cond.Id());
} else {
auto& r_elem_master = mOriginModelPart.GetElement(master_id); 
...
IndexSet::Pointer p_indexes_pairs = r_elem.GetValue( INDEX_SET ); 
for (auto it_pair = p_indexes_pairs->begin(); it_pair != p_indexes_pairs->end(); ++it_pair ) {
const IndexType master_id = p_indexes_pairs->GetId(it_pair);
if (mOptions.Is(ORIGIN_SKIN_IS_CONDITION_BASED)) {
auto& r_cond_master = mOriginModelPart.GetCondition(master_id); 
...
IndexMap::Pointer p_indexes_pairs = r_elem.GetValue( INDEX_MAP ); 
for (auto it_pair = p_indexes_pairs->begin(); it_pair != p_indexes_pairs->end(); ++it_pair ) {
const IndexType master_id = p_indexes_pairs->GetId(it_pair);
if (mOptions.Is(ORIGIN_SKIN_IS_CONDITION_BASED)) {
auto& r_cond_master = mOriginModelPart.GetCondition(master_id); 

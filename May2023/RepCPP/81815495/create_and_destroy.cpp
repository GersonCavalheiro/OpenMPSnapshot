...
pnew_node->FastGetSolutionStepValue(CHARACTERISTIC_LENGTH) = radius * 2.0; 
...
const bool is_breakable = (*r_params)[BREAKABLE_CLUSTER]; 
...
if (r_balls_model_part.NumberOfElements(0)) { 
...
if (r_rigid_faces_model_part.NumberOfConditions(0)) { 
...
if (r_dem_inlet_model_part.NumberOfNodes(0)) { 
Configure::NodesContainerType Nodes = r_dem_inlet_model_part.GetCommunicator().LocalMesh().Nodes();

if (r_balls_model_part.NumberOfElements(0) == 0 && r_rigid_faces_model_part.NumberOfConditions(0) == 0) { 

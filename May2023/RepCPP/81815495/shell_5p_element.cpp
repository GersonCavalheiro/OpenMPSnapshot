...
ConstitutiveVariables constitutive_variables(8); 
...
for (int s = 0; s < 3; s++)
{
rB(0, kr + s) = m_cart_deriv[iP](0, r) * rActualKinematic.a1[s]; 
...
rB(3, kr + s) = m_cart_deriv[iP](0, r) * rActualKinematic.dtd1[s]; 
...
rB(6, kr + s) = m_cart_deriv[iP](0, r) * rActualKinematic.t[s];  
rB(7, kr + s) = m_cart_deriv[iP](1, r) * rActualKinematic.t[s];
}

const Vector Temp = prod(prod<Vector>(trans(rActualKinematic.a1), WI1), BLAI); 
...
const Vector Temp3 = prod(prod<Vector>(trans(rActualKinematic.a1), rVariations.P) * r_N(iP, r), BLAI); 
...
const Matrix3d chiAndSfac = ractVar.Chi11 * S[3] + ractVar.Chi22 * S[4] + ractVar.Chi12Chi21 * S[5]  
+ ractVar.S1 * S[6] + ractVar.S2 * S[7]; 
...
Kg(i1, j1) = Kg(i1 + 1, j1 + 1) = Kg(i1 + 2, j1 + 2) = NS; 

Matrix3d Temp = S[3] * dN1i * WJ1 + S[4] * dN2i * WJ2 + S[5] * (dN1i * WJ2 + dN2i * WJ1); 
Temp += ractVar.P * Nj * (dN1i * S[6] + dN2i * S[7]);  

noalias(subrange(Kg, i1, i1 + 3, j4, j4 + 2)) = prod(Temp, BLAJ);
Temp = S[3] * dN1j * WI1 + S[4] * dN2j * WI2 + S[5] * (dN1j * WI2 + dN2j * WI1); 

Temp += ractVar.P * Ni * (dN1j * S[6] + dN2j * S[7]);

noalias(subrange(Kg, i4, i4 + 2, j1, j1 + 3)) = prod(BLAI_T, Temp);

const double NdN1 = dN1j * Ni + Nj * dN1i;
const double NdN2 = dN2j * Ni + Nj * dN2i;
Temp = Ni * Nj * chiAndSfac;  
Temp += ractVar.S1 * NdN1 * S[3] + ractVar.S2 * NdN2 * S[4] + (ractVar.S1 * NdN2 + ractVar.S2 * NdN1) * S[5];  

const Matrix23d Temp2 = prod(BLAI_T, Temp); 
...
prod(ractVar.P, rActKin.a2) * Ni * S[7]); 
Kg(i1 + 3, i1 + 3) += kgT;
Kg(i1 + 4, i1 + 4) += kgT;
}

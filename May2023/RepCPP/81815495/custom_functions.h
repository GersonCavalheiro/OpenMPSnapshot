...
array_1d <double, TDim> grad_aux = prod(trans(DN_DX), elemental_pressures); 
...
double max_pressure_change_rate = 0.0; 
...
const double characteristic_length             = std::pow(mTotalDomainVolume, 1.0 / 3); 
...
if (std::abs(characteristic_pressure_variation) == 0.0 || std::abs(reciprocal_of_characteristic_time) == 0.0){ 

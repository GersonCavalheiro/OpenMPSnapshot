...
for(auto J : *it )
{
if(rReferenceGraph.find({I,J}) == rReferenceGraph.end()) 
...
const SparseTestingInternals::IndexType nel = 1e2; 
...
auto pAconverted = AmgclCSRConversionUtilities::ConvertToCsrMatrix<double,IndexType>(*pAmgcl); 
...
CsrMatrix<double>::Pointer pC = AmgclCSRSpMMUtilities::SparseMultiply(A,*pAconverted); 
}
...
CsrMatrix<double>::Pointer pC = AmgclCSRSpMMUtilities::SparseMultiply(A,B); 
auto& C = *pC;
...
SystemVector<> y(A.size1()); 
y.SetValue(0.0);

SystemVector<> x(A.size2()); 
x.SetValue(1.0);
...
SystemVector<> xtranspose_spmv_ref(A.size2()); 

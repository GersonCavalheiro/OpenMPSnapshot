...
const bool is_time_to_search_neighbours = (time_step + 1) % mNStepSearch == 0 && (time_step > 0); 
...
fem_model_part.AddElement(RigidBodyElement3D_Kratos); 
submp.AddElement(RigidBodyElement3D_Kratos); 
...
if (submp.Has(TABLE_NUMBER_VELOCITY)) { 
...
if (submp.Has(TABLE_NUMBER_ANGULAR_VELOCITY)) { 
...
if (submp.Has(EXTERNAL_APPLIED_FORCE)) { 
...
if (submp.Has(EXTERNAL_APPLIED_MOMENT)) { 
...
if (submp.Has(TABLE_NUMBER_FORCE)) { 
...
if (submp.Has(TABLE_NUMBER_MOMENT)) { 
...
if (neighbour_element == NULL) continue; 
...
if (neighbour_element->mNeighbourElements[k] == NULL) continue; 
...
double max_indentation = std::max(0.0, 0.5 * indentation); 

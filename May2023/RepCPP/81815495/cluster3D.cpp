...
unsigned int* p_max_Id = p_creator_destructor->pGetCurrentMaxNodeId();  
...
Node& central_node = GetGeometry()[0]; 
...
if (mListOfSphericParticles[i]->mNeighbourElements.size()==0 && mListOfSphericParticles[i]->mNeighbourRigidFaces.size()==0) continue; 

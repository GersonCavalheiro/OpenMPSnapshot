...
array_1d <double, 3> grad_aux = prod(trans(DN_DX), elemental_values); 
...
grad *= nodal_area;
for (unsigned int i = 0; i < TDim + 1; ++i){
geom[i].FastGetSolutionStepValue(material_derivative_container) += grad; 
}
}
for (NodeIteratorType inode = r_model_part.NodesBegin(); inode != r_model_part.NodesEnd(); ++inode){
...
std::vector<array_1d <double, 3> > polynomial_coefficients; 
...
array_1d <double, TDim> grad_aux = prod(trans(DN_DX), elemental_values); 
for (unsigned int i = 0; i < TDim; ++i){
grad[i] = grad_aux[i];
}
double nodal_area = Volume / static_cast<double>(TDim + 1);
...
double Volume;
GeometryUtils::CalculateGeometryData(geom, DN_DX, N, Volume);
array_1d <double, 3> average = ZeroVector(3); 
for (unsigned int i = 0; i < TDim; ++i){
...
array_1d <double, TDim + 1 > N; 
BoundedMatrix<double, TDim + 1, TDim> DN_DX;
BoundedMatrix<double, TDim + 1, TDim> elemental_vectors; 
...
grad *= nodal_area;
for (unsigned int i = 0; i < TDim + 1; ++i){
geom[i].FastGetSolutionStepValue(laplacian_container) += grad; 
}
}
for (NodeIteratorType inode = r_model_part.NodesBegin(); inode != r_model_part.NodesEnd(); ++inode){
...
for (unsigned int i = 0; i < TDim + 1; ++i){
for (unsigned int k = 0; k < TDim; ++k){
elemental_vectors(i, k) = geom[i].FastGetSolutionStepValue(laplacian_container)[k]; 
}
}
BoundedMatrix<double, TDim, TDim> grad_aux = prod(trans(DN_DX), elemental_vectors); 
...
for (unsigned int i = 0; i < TDim + 1; ++i){
laplacians[id_to_position[geom[i].Id()]][j] += divergence_of_vi; 
}
}
for (NodeIteratorType inode = r_model_part.NodesBegin(); inode != r_model_part.NodesEnd(); ++inode){
...
{
unsigned int n_poly_terms = Factorial(TDim + 2) / (2 * Factorial(TDim)); 
...
const double h_inv = 1.0 / CalculateTheMaximumDistanceToNeighbours(p_node); 
const array_1d <double, 3> origin = p_node->Coordinates();

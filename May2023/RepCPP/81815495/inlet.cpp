...
mp[INLET_INITIAL_VELOCITY] = mp[LINEAR_VELOCITY];    
mp[INLET_INITIAL_PARTICLES_VELOCITY] = mp[VELOCITY]; 
...
for (unsigned int j = 0; j < r_cluster.GetSpheres().size(); j++) { 
...
const double delta_t = current_time - mLastInjectionTimes[smp_number]; 
double surface = 1.0; 
...
number_of_particles_to_insert = mesh_size_elements; 
...
const double minimum_velocity = mean_radius * 3.0 / maximum_time_until_release; 
...
if (double_number_of_particles_to_insert < INT_MAX){ 
...
ModelPart::ElementsContainerType::ContainerType valid_elements(mesh_size_elements); 
...
valid_elements[random_pos]->Set(ACTIVE); 
valid_elements[random_pos]->GetGeometry()[0].Set(ACTIVE);
valid_elements[random_pos] = valid_elements[valid_elements_length - 1]; 
valid_elements_length--; 
}

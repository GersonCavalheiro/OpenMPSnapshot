...
if (mVariables.Is(variable, "FluidTimeFiltered") && variable != FLUID_FRACTION){ 
...
if (mVariables.Is(PARTICLE_VEL_FILTERED, "FluidTimeFiltered") && variable == PARTICLE_VEL_FILTERED){ 
ApplyExponentialTimeFiltering(r_fluid_model_part, PARTICLE_VEL_FILTERED, TIME_AVERAGED_ARRAY_3);
}
if (mVariables.Is(GetBodyForcePerUnitMassVariable(), "FluidTimeFiltered") && variable == GetBodyForcePerUnitMassVariable()){ 
...
p_elem->GetGeometry()[i_nearest_node].FastGetSolutionStepValue(PHASE_FRACTION) += particle_mass; 
}
}
template <std::size_t TDim, typename TBaseTypeOfSwimmingParticle>
...
for (unsigned int i = 0; i < TDim + 1; ++i){
p_elem->GetGeometry()[i].FastGetSolutionStepValue(PHASE_FRACTION) += N[i] * particle_mass; 
}
}
}
template <std::size_t TDim, typename TBaseTypeOfSwimmingParticle>
...
template <std::size_t TDim, typename TBaseTypeOfSwimmingParticle>
void BinBasedDEMFluidCoupledMapping<TDim, TBaseTypeOfSwimmingParticle>::CalculateNodalFluidFractionByAveraging( 
ParticleType& particle,
const ResultNodesContainerType& neighbours,
const DistanceType& weights)
{
...
if (mTimeAveragingType == 1 && mNumberOfDEMSamplesSoFarInTheCurrentFluidStep == 0){ 
...
{
if (mIsFirstTimeFiltering[r_variable]){ 

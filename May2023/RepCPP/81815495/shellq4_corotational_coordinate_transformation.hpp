...
noalias(rLeftHandSideMatrix) += prod(Fnm, G);     
...
ShellQ4_LocalCoordinateSystem b(nodes[0], nodes[1], nodes[2], nodes[3]);   
...
ShellQ4_LocalCoordinateSystem c(nodes[0], nodes[1], nodes[2], nodes[3], alpha);   


noalias(e3) = c.Vz();
noalias(e1) = c.Vx();

G(0, index + j) = -e3(1) / pert; 
G(1, index + j) =  e3(0) / pert; 
G(2, index + j) =  e1(1) / pert; 

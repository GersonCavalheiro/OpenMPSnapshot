...
const double initial_volume = this->ComputeInitialVolumeSubModel(r_sub_model_part); 
...
if (rSubModelName.size() == ref_string_size) { 
return std::stoi(rSubModelName.substr(string_size - 1, string_size));
} else if (rSubModelName.size() == ref_string_size + 1) { 

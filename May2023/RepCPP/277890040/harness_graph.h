...
static tbb::flow::continue_msg convert_value(const InputType &) {
...
static OutputType convert_value(const tbb::flow::continue_msg &) {
return OutputType();
}
};
...
typename RW::scoped_lock l( mutex_holder<RW>::mutex, false );
return func(v);
}
...
typename RW::scoped_lock l( harness_graph_executor::mutex_holder<RW>::mutex, false );
...
typename RW::scoped_lock l( harness_graph_multifunction_executor::mutex_holder<RW>::mutex, false );
my_execute_count.fetch_and_increment();
...
#endif
void reset_receiver(tbb::flow::reset_flags ) __TBB_override { my_count = 0; }
};
...
size_t predecessor_count() __TBB_override { return 0; }
#endif
void reset_receiver(tbb::flow::reset_flags ) __TBB_override {
my_count = 0;
if(my_map) delete my_map;
my_map = new map_type;
}
...
T operator()(const tbb::flow::continue_msg& ) {
*_flag = 1;
tbb::internal::atomic_backoff backoff;
do {
backoff.pause();
...
ASSERT(2 == (int)outt || 1 == (int)outt, " buffer had incorrect message");  
...
test_buffer_base_extract& operator=(test_buffer_base_extract & );
...
private:
void increase_and_check(const tbb::tbb_thread::id& ) {
++my_body_count;

...
int set_Laplacian_coefficients(real_t c[], 
int const nn=1, 
...
double clear_diagonal_elements() { 
double diag{0};
for (int d = 0; d < 3; ++d) {
diag += c2nd[d][0];
c2nd[d][0] = 0; 
...
template <typename complex_out_t 
,typename complex_in_t 
,typename real_fd_t> 
status_t apply(
complex_out_t out[]
...
int const n16 = nnArraySize; 
typedef int16_t int_t;
std::vector<int_t> list[3]; 
...
assert(nf <= n);
assert(nf <= n16);
int const nh = n16 + n + n16; 
list[d] = std::vector<int_t>(nh, -1); 
phas[d] = std::vector<complex_in_t>(nh, 0); 
...
if (Periodic_Boundary == bc) { 
for (int j = -nf; j < 0; ++j) {
list[d][n16 + j] = (n + j) % n; 
phas[d][n16 + j] = phase_low; 
} 
} else if (Mirrored_Boundary == bc) { 
for (int j = -nf; j < 0; ++j) {
list[d][n16 + j] = - 1 - j; 
phas[d][n16 + j] = phase_low; 
} 
...
if (Periodic_Boundary == bc) { 
for (int j = 0; j < nf; ++j) {
list[d][n16 + n + j] = (n + j) % n; 
phas[d][n16 + n + j] = phase_upp; 
} 
} else if (Mirrored_Boundary == bc) { 
for (int j = 0; j < nf; ++j) {
list[d][n16 + n + j] = n - 1 - j; 
phas[d][n16 + n + j] = phase_upp; 
} 
} 
...
status_t stat(0);
int const mantissa_bits = (sizeof(real_t) > 4)? 52 : 23; 
...
if (checksum > maxdev) { maxdev = checksum; maxdev_nn = nn; } 
} 
...
double const k = (1 + dir + iphase/360.)*2*constants::pi/g[dir]; 
...
recorded_warnings::clear_warnings(); 

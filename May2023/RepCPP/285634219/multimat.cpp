...
#pragma omp parallel for
for (int j = mat*width-(mat>0)-(mat>0)*overlap_j; j < (mat+1)*width; j++) { 
...
#pragma omp parallel for
for (int j = sizey-mat*width-1+(mat>0)*overlap_j; j >= sizey-(mat+1)*width-(mat<(cc.Nmats/2-1)); j--) { 
...
cc.n = (double*)malloc(Nmats*sizeof(double)); 
...
ccc.mmc_index = (int *)hbw_malloc(list_size*sizeof(int)); 
ccc.mmc_i = (int *)hbw_malloc(list_size*sizeof(int)); 
ccc.mmc_j = (int *)hbw_malloc(list_size*sizeof(int)); 

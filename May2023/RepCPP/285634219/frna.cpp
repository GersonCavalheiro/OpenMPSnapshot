...
{
#ifdef OMP_TARGET
#pragma omp target teams num_teams(n) thread_limit(SQRT_NTHREAD*SQRT_NTHREAD)
{
int_t buf[NTHREAD];
#pragma omp parallel
{
...
for (int d1 = d1start; d1 <= d1max; d1 += d1inc) { 
const int ip = i+d1+1; 
...
#ifdef OMP_TARGET
free_energy_min_reduce(buf, &vij, omp_get_thread_num(), omp_get_num_threads());

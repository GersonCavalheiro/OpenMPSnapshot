...
#pragma omp parallel for
for(int i=0;i<6;i++){
newPos->_Cases[0][16 * i]=pos->_Cases[0][16 * i];
newPos->_Cases[1][16 * i]=pos->_Cases[1][16 * i];
}
...
const int nbpions=pos->_Cases[joueur][16 * coup];
if (nbpions == 0){
return 0; 
}
copier(newPos,pos);
newPos->_Cases[joueur][16 * coup]=0;
int j=joueur;
int c=coup;
...
int nbp=newPos->_Cases[joueur][16 * coup];
while (nbp != 0){
newPos->_Cases[joueur][16 * coup]=0;
for(int i=0;i<nbp;i++){
const int tj=j;
...
nbp=newPos->_Cases[joueur][16 * coup];
}
if (j != joueur){
if (j ==0){
for(int i=c;i<=5;i++){

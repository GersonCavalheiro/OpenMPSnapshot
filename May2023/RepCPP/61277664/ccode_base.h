...
counter[2] += difference_num(n0n1y2, y1); 
if (v2 > v1) continue;
counter[5] += intersection_num(y0y1, y2, v2); 
}
...
VertexSet y2 = g.N(v2);
counter[3] += intersection_num(n0y1, y2, v0); 
}
}
}
}
...
for (auto v3 : y0y1) {
if (v3 < v2 && local_ccodes[v3] == 3) counter[4] ++; 
}
VertexSet y2 = g.N(v2);
for (auto v3 : y2) {
...
for (auto v3 : y0y1) {
if (v3 > v2) break;
if (local_ccodes[v3] == 7) counter[5] ++; 
}
}

#ifndef OMPInterpolation_h
#define OMPInterpolation_h
...
{
int i, j;
#pragma omp parallel for private(j, i) ordered
for (j = 0; j < h; j += 2)
{
for (i = 0; i < w; i += 2)
{
...
for (int i = 0; i < w; i++)
{
r_mat161.at<short>(j, i) = r_data[j * w + i];
g_mat161.at<short>(j, i) = g_data[j * w + i];
b_mat161.at<short>(j, i) = b_data[j * w + i];
}
}
...
void SaveRawFile10bit(unsigned short *_data)
{
FILE *fp;
fp = fopen("../result/omp.raw", "wb");
fwrite(_data, sizeof(unsigned short), h * w * 3, fp);
fclose(fp);
}

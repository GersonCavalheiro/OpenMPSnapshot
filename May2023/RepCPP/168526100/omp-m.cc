...
int *image = new int[width * height + 1];

double unit_y = ((upper - lower) / height);
double unit_x = ((right - left) / width);

#pragma omp parallel for schedule(dynamic, width / 2)
...
double program_time = duration_cast<duration<double>>(program_end - program_begin).count() - io_time;
std::cout << program_time << " " << io_time;
}

...
unsigned int spacetime_size = ( has_space_time_AM ? 2*params.nmodes+1 : 2 );
...
for (unsigned int imode=0; imode<spacetime_size/2; imode++){ 
name.str( "" );
...
unsigned int dim = nr_p + nr_d; 

vector<double> pos( 1 );
for( unsigned int j=0 ; j<dim ; j++ ) {
pos[0] = patch->getDomainLocalMin( 1 ) + ( j*0.5 - 0.5 - oversize[1] )*dr ; 
( *space_envelope )( j, 0 ) = spaceProfile_->valueAt( pos );
( *phase )( j, 0 ) = phaseProfile_->valueAt( pos );
}

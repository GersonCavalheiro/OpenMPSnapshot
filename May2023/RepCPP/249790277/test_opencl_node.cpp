...
k.set_args( port_ref<0, 1>(),  1,  0,  0,  1, err, err_size );
...
int stride_x = 1;
k.set_args( port_ref<0>(), BROKEN_FUNCTION_POINTER_DEDUCTION( port_ref<1, 1> ), stride_x,  1024,  0,  2, err, err_size );
...
k.set_args( port_ref<0>(), BROKEN_FUNCTION_POINTER_DEDUCTION( port_ref<1> ),  1,  64,  stride_z,  3, err, err_size );
...
k.set_args( port_ref<0, 1>(),  2,  2 * 64,  2 * 64 * 64,  3, err, err_size );
...
k.set_args( port_ref<0, 1>(),  1,  1024,  0,  2, err, err_size );
...
bool res = !KeyMatchingTest<int, queueing>(); 

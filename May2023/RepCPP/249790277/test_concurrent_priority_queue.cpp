...
template <typename T, typename C>
void TestParallelPushPop(int nThreads, T t_max, T t_min, C ) {
size_t qsize;
...
template <typename T, typename C>
void TestFlogger(int nThreads, T , C ) {
...
typedef two_memory_arenas_fixture<MoveOperationTracker, Harness::true_type> arena_fixture_t;
typedef cpq_src_fixture<arena_fixture_t::allocator_t > fixture_t;
...
typedef two_memory_arenas_fixture<MoveOperationTracker, Harness::false_type> arena_fixture_t;
typedef cpq_src_fixture<arena_fixture_t::allocator_t > fixture_t;
...
Examine<false>(q1, q1Copy, vecSorted);
Examine<false>(q2, q2Copy, vecSorted);
...
Examine<false>(q3, q3Copy, vecSorted);
Examine<false>(q4, q4Copy, vecSorted);
#endif 
}
#endif 

...
for( int i=1; i<10; ++i ) {
tbb::tick_count::interval_t intv((double)i*0.0999 );
ul.lock();
...
for( int i=1; i<10; ++i ) {
tbb::tick_count::interval_t intv((double)i*0.0001 );
ul.lock();
__TBB_TRY {
...
while( n_waiters<(nthread-1) ) __TBB_Yield(); 
...
while( n_done_2<nthread+1 ) __TBB_Yield(); 

tbb::tick_count::interval_t intv((double)2.0 );
ul.lock();
++n_waiters; 
t1 = tbb::tick_count::now();
cv_status st = test_cv.wait_for( ul, intv ); 
...
while( n_waiters<(nthread-1) ) __TBB_Yield(); 
for( int i=0; i<2*short_delay; ++i ) __TBB_Yield();  
ul.lock();
false_to_true = true;
test_cv.notify_all(); 
ul.unlock();

while( n_done<nthread ) __TBB_Yield(); 
ASSERT( n_signaled>0, "too small an interval?" );
} else {
...
++n_waiters;
t1 = tbb::tick_count::now();
ASSERT( test_cv.wait_for( ul, intv, TestPredicateFalseToTrue())==true, NULL ); 
t2 = tbb::tick_count::now();
ul.unlock();
...
int my_ticket = ++ticket_for_wakeup; 
...
unique_lock<M> ul( my_mtx, defer_lock );
ul.lock();
int my_ticket = ++ticket_for_sleep; 

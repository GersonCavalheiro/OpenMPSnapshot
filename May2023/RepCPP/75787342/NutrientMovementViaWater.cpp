...
ww = -mw / satportion;
vno3 = m_soilNO3[i][k] * (1. - CalExp(ww)); 
...
cosurf = m_nperco * con;
if (k == 0) {
m_surfRfNO3[i] = m_surfRf[i] * cosurf; 
m_surfRfNO3[i] = Min(m_surfRfNO3[i], m_soilNO3[i][k]);
m_soilNO3[i][k] -= m_surfRfNO3[i];
}
if (m_drainLyr[i] == k) {
}
FLTPT ssfnlyr = k == 0 ? cosurf * m_subSurfRf[i][k] : con * m_subSurfRf[i][k]; 
...
if (qdr > 1.e-4) {
FLTPT tp = 100. * (m_sedorgn[i] + m_surfRfNO3[i]) / qdr; 
FLTPT chla_subco = 40.;
m_surfRfChlA[i] = chla_subco * tp;
m_surfRfChlA[i] *= 0.001; 
...
FLTPT cbodu = 2.7 * org_c / (qdr * m_cellWth * m_cellWth * 1.e-6); 
FLTPT cod = m_cod_n * (cbodu * (1. - CalExp(-5. * m_cod_k)));
m_surfRfCod[i] = m_surfRf[i] * 0.001 * cod * 10.; 
} else {
m_surfRfChlA[i] = 0.;
m_surfRfCod[i] = 0.;
}

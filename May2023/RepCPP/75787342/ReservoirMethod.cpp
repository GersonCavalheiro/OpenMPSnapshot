...
CheckInputData();
InitialOutputs();
FLTPT QGConvert = 1. * m_cellWth * m_cellWth / m_dt * 0.001; 
...
FLTPT groundRunoff = kg * CalPow(m_gwSto[subID], m_Base_ex); 
FLTPT groundQ = groundRunoff * curCellsNum * QGConvert;     
...
groundQ = groundRunoff * curCellsNum * QGConvert; 
...
m_T_RG[subID] = curSub->GetRg(); 
m_T_QG[subID] = curSub->GetQg(); 
...
m_T_RG[0] = m_subbasinsInfo->Subbasin2Basin(VAR_RG[0]); 
m_T_QG[0] = m_subbasinsInfo->Subbasin2Basin(VAR_QG[0]); 

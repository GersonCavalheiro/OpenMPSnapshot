...
consensus_a[tick] = seq_a[curr.row - 1];   					
}

if ( next.col == curr.col ) { 
consensus_b[tick] = '-';                  					
} else {
consensus_b[tick] = seq_b[curr.col - 1];   					
}
...
mscore = _mm256_broadcast_ss( constants ), 				
nscore = _mm256_broadcast_ss(constants+1),				
dvect  = _mm256_broadcast_ss(constants+2),				
zvect  = _mm256_broadcast_ss(constants+3);				
#else 

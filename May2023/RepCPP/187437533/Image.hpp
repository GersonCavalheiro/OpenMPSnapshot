...
void SetPixelW(const double X, const double Y, const Colour C) 
{
...
std::string Path = "Out/";
std::string NumStr = std::to_string(ImgID); 
...
if (ThreadID == 0)
{
std::cout << "(t0) Wrote image file: " << Filename << "\r" << std::flush; 
}
}
};

...
tcap[m-1]=iter;
#pragma omp parallel for shared(p,P)
for(int j=0;j<N;j++)
P[m-1][j]=p[iter&1][j];
}
...
#pragma omp parallel for schedule(auto) shared(lambda,sigma,ccov)
...
#pragma omp prallel for shared(x,m,sigma,ccov,p,z)
for(int n=0; n <N;n++){
z[n]=distribution(gen);
int val = 0;
#pragma omp prallel for shared(ccov,P) reduction(+:val) 
for(int j=0;j<M;j++){
...
#pragma omp parallel for shared(mu,x)
for(int i = 0;i<mu;i++){
F[(t+1)&1][i] = func(x[i],num);
}

#pragma omp parallel for shared(mu,x,w,p,m,sigma,cc,mueff)
for(int n=0;n<N;n++){
double mnew=0;
#pragma omp parallel for shared(mu,x,w,n) reduction(+:mnew)
for(int i=0;i<mu;i++){
mnew += w[i]*x[i][n];
}
m[(t+1)&1][n] = mnew; 

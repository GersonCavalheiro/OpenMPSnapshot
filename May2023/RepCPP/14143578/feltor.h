...
double& bp0, double& bp1, double& bp2, 
...
void compute_phi( double t, const std::array<Container,2>&, Container&);
void compute_psi( double t, const Container& phi, Container& psi);
...
m_b[0], m_b[1], m_b[2]);
dg::assign( m_b[2], m_bphi); 
m_detg = dg::tensor::volume( metric);
dg::blas1::pointwiseDivide( m_binv, m_detg, m_temp0); 
for( int i=0; i<3; i++)
dg::blas1::pointwiseDot( m_temp0, m_b[i], m_b[i]); 
...
m_dxC(   dg::create::dx( g, dg::NEU, dg::centered) ), 
m_dyC(   dg::create::dy( g, dg::NEU, dg::centered) ), 
...
double time, const Container& phi, Container& psi
)
{
if (m_p.tau[1] == 0.) {
...
m_dP[0][0], m_dP[0][1], m_dP[0][2], m_hh, m_binv, 
...
compute_phi( t, m_density, m_potential[0]);
compute_psi( t, m_potential[0], m_potential[1]);
...
for( unsigned i=0; i<2; i++)
{
multiply_rhs_penalization( yp[i]); 
dg::blas1::pointwiseDot( -m_wall_rate, m_wall, density[i],
-m_sheath_rate, m_sheath, density[i], 1., yp[i]); 
}
}
...
-m_sheath_rate, m_sheath, velocityST[i], 1., yp[i]); 
}
}

...
{
dg::blas1::axpby( m_p.mu[1], nme[1], m_p.mu[1], 1., m_temp0); 
...
{
m_arakawa( y[0][i], m_phi[i], yp[0][i]);                 
m_arakawa( y[1][i], m_phi[i], yp[1][i]);                 

dg::blas1::pointwiseDot( 1., m_n[i], m_u[i], 0., m_temp0);
m_arakawa( 1., m_apar[i], m_temp0,   1., yp[0][i]); 

dg::blas1::transform( m_n[i], m_temp0, dg::LN<double>());
m_arakawa( m_p.tau[i]/m_p.mu[i], m_apar[i], m_temp0, 1., yp[1][i]);  
dg::blas1::pointwiseDot( 1., m_u[i], m_u[i], 0., m_temp0);
m_arakawa( 0.5, m_apar[i], m_temp0,   1., yp[1][i]);                       
}
}
...
compute_phi( time, y[0]); 

compute_psi( time ); 

dg::blas1::transform( y[0], m_n, dg::PLUS<>(+1.)); 

...
#pragma omp target depend(inout: h_array) depend(in: in_1) depend(in: in_2) map(tofrom: isHost) map(alloc: in_1[0:N]) map(alloc: in_2[0:N]) map(alloc: h_array[0:N])
{
isHost = omp_is_initial_device();
for (int i = 0; i < N; ++i) {
h_array[i] = DEVICE_TASK1_BIT | in_1[i] | in_2[i]; 

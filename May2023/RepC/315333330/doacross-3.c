...
#pragma omp atomic write
b[i][j][k] = 1;
#pragma omp ordered depend(sink: i, j - 2, k - 1) depend(sink: i - 2, j - 2, k + 1)
...
#pragma omp ordered depend (sink: i, j, k + 2 E(m)) depend (sink:i - 2, j + 1, k - 4 E(m)) depend(sink: i - 1, j - 2, k - 2 E(m))
if (k <= 4)
{
#pragma omp atomic read
l = c[i][j][k + 2];
...
#pragma omp atomic write
g[j][k][i] = 1;
#pragma omp ordered depend(sink: j, k - 2, i - 1) depend(sink: j - 2, k - 2, i + 1)

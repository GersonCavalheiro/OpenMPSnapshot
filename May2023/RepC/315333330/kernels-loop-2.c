...
#pragma acc kernels copyout (b[0:N]) num_gangs (3 + a[3]) num_workers (5 + a[5]) vector_length (7 + a[7])
{
for (COUNTERTYPE i = 0; i < N; i++)
b[i] = i * 4;
}
#pragma acc kernels copyin (a[0:N], b[0:N]) copyout (c[0:N]) num_gangs (3) num_workers (5) vector_length (7)
{

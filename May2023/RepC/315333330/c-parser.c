...
{
int des_seen = 0;
location_t des_loc = UNKNOWN_LOCATION;  
...
location_t label_loc = UNKNOWN_LOCATION;  
if (c_parser_next_token_is (parser, CPP_CLOSE_BRACE))
{
add_debug_begin_stmt (c_parser_peek_token (parser)->location);
c_parser_consume_token (parser);
...
c_parser_declaration_or_fndef (parser,  false,
true,
true,  true,
true, NULL,
vNULL);
...
int sp;
location_t binary_loc = UNKNOWN_LOCATION;  
#define POP								      \
do {									      \
switch (stack[sp].op)						      \
{									      \

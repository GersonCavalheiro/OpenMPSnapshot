...
ACCESS_REGS,	ACCESS_REGS, VEC_REGS, VEC_REGS,  
...
else if (DISP_IN_RANGE (INTVAL (offset)))               
{
...
{
if (!satisfies_constraint_K (offset)                
&& (!TARGET_EXTIMM
|| (!satisfies_constraint_Op (offset)       
&& !satisfies_constraint_On (offset)))) 
offset = force_const_mem (Pmode, offset);
if (target != reg)
{
insn = emit_move_insn (target, reg);

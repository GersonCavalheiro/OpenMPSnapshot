...
#pragma acc enter data copyin(length[0:N]) copyin(index[0:elements], data[0:elements])
break;
}
default:
...
int *index = matrixELL->index;
floatType *data = matrixELL->data;
#pragma acc exit data delete(length[0:N]) delete(index[0:elements], data[0:elements])
break;
}
default:
assert(0 && "Invalid matrix format!");
...
int *index = matrixCRS->index;
floatType *value = matrixCRS->value;
#pragma acc parallel loop gang vector present(x[0:N], y[0:N]) present(ptr[0:N+1], index[0:nz], value[0:nz])
for (int i = 0; i < N; i++) {
floatType tmp = 0;
...
int *index = matrixELL->index;
floatType *data = matrixELL->data;
#pragma acc parallel loop gang vector present(x[0:N], y[0:N], length[0:N]) present(index[0:elements], data[0:elements])
for (int i = 0; i < N; i++) {
floatType tmp = 0;

...
floatType *x = this->x;
floatType *k = this->k;
#pragma omp target enter data map(alloc: p[0:N], q[0:N], r[0:N]) map(to: x[0:N], k[0:N])
switch (matrixFormat) {
...
#pragma omp target enter data map(to: length[0:N]) map(to: index[0:elements], data[0:elements])
break;
}
default:
...
floatType *x = this->x;
floatType *k = this->k;
#pragma omp target exit data map(release: p[0:N], q[0:N], r[0:N], k[0:N]) map(from: x[0:N])
switch (matrixFormat) {
case MatrixFormatCRS: {
int *ptr = matrixCRS->ptr;
int *index = matrixCRS->index;
floatType *value = matrixCRS->value;
#pragma omp target exit data map(release: ptr[0:N+1]) map(release: index[0:nz], value[0:nz])
break;
}
case MatrixFormatELL: {
int elements = matrixELL->elements;
...
int *index = matrixELL->index;
floatType *data = matrixELL->data;
#pragma omp target exit data map(release: length[0:N]) map(release: index[0:elements]) map(release: data[0:elements])
break;
}
default:
assert(0 && "Invalid matrix format!");
...
int *index = matrixCRS->index;
floatType *value = matrixCRS->value;
#pragma omp target teams distribute parallel for map(x[0:N], y[0:N]) map(ptr[0:N+1], index[0:nz], value[0:nz])
for (int i = 0; i < N; i++) {
floatType tmp = 0;
...
int *index = matrixELL->index;
floatType *data = matrixELL->data;
#pragma omp target teams distribute parallel for simd map(x[0:N], y[0:N]) map(length[0:N], index[0:elements], data[0:elements])
for (int i = 0; i < N; i++) {
floatType tmp = 0;
#pragma unroll 1
...
#pragma omp target teams distribute parallel for simd reduction(+:res) map(res, a[0:N], b[0:N])
for (int i = 0; i < N; i++) {
res += a[i] * b[i];
}
return res;
...
int N = this->N;
floatType *C = jacobi->C;
#pragma omp target teams distribute parallel for simd map(x[0:N], y[0:N], C[0:N])
for (int i = 0; i < N; i++) {
y[i] = C[i] * x[i];
}
}

...
#pragma acc enter data async copyin(length[0:N], index[0:elements]) copyin(data[0:elements])
}
...
#pragma acc enter data async create(p[0:N], q[offset:length]) create(r[offset:length]) copyin(x[0:N], k[offset:length])
switch (matrixFormat) {
...
int *index = matrix.index;
floatType *data = matrix.data;
#pragma acc exit data async delete(length[0:N], index[0:elements]) delete(data[0:elements])
}
...
#pragma acc exit data async delete(p[0:N], q[offset:length]) delete(r[offset:length], k[offset:length]) delete(x[0:N])
switch (matrixFormat) {
case MatrixFormatCRS:
if (!overlappedGather) {
...
#pragma acc parallel loop async gang vector present(x[0:N], y[offset:length]) present(ptr[0:length+1], index[0:nz], value[0:nz])
...
#pragma acc parallel loop async gang vector present(x[0:N], y[offset:length]) present(lengthA[0:length], index[0:elements]) present(data[0:elements])
for (int i = 0; i < length; i++) {
...
#pragma acc parallel loop async present(a[offset:length], b[offset:length]) present(res_dev) reduction(+:res_dev)
for (int i = offset; i < offset + length; i++) {
res_dev += a[i] * b[i];
}
}
...
int offset = workDistribution->offsets[d];
int length = workDistribution->lengths[d];
#pragma acc parallel loop async present(x[offset:length], y[offset:length]) present(C[offset:length])
for (int i = offset; i < offset + length; i++) {
y[i] = C[i] * x[i];
}
}

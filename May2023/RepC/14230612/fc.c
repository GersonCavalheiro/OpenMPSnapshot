...
#pragma omp parallel for default(none) private (rowcol, tempval) shared (N, minval, maxval, l2)
...
#pragma omp parallel for default(none) private (rowcol, tempval) shared (N, minval, maxval, l2, lOut)
for(rowcol=0;rowcol<N;rowcol++){
if( l2[rowcol] == -28768){
lOut[rowcol] = 0;
} 

...
#pragma omp parallel for default(none) private (rowcol, tempval) shared (N, minval, maxval, l2, l3)
...
#pragma omp parallel for default(none) private (rowcol, tempval) shared (N, minval, maxval, l2, l3, lOut)
for(rowcol=0;rowcol<N;rowcol++){
if( l2[rowcol] == 0 || l3[rowcol] == 0){
lOut[rowcol] = 0;
} 

...
for (v = 0; v < cache->numPropertyRegions; ++v)
{
cache->propertyRegions[v].fraction    = property_fraction; 
...
{
float missRate = (double)((getWM(cache) + getRM(cache)) * 100) / (getReads(cache) + getWrites(cache)); 
...
float missRate = (double)((getWM(cache) + getRM(cache)) * 100) / (getReads(cache) + getWrites(cache)); 
missRate       = roundf(missRate * 100) / 100;                                                
float missRateRead = (double)((getRM(cache)) * 100) / (getReads(cache));   
missRateRead       = roundf(missRateRead * 100) / 100;                     
float missRateWrite = (double)((getWM(cache)) * 100) / (getWrites(cache)); 
missRateWrite       = roundf(missRateWrite * 100) / 100;                   
printf(" -----------------------------------------------------\n");
...
float missRate = (double)(ReadWriteMisses_total * 100) / (ReadWrite_total); 
missRate       = roundf(missRate * 100) / 100;
return missRate;
}
...
float missRate = (double)(ReadWriteMisses_total * 100) / (ReadWrite_total); 
missRate       = roundf(missRate * 100) / 100;
float missRateRead = (double)((ReadMisses_total) * 100) / (Read_total); 
missRateRead       = roundf(missRateRead * 100) / 100;
float missRateWrite = (double)((WriteMisses_total) * 100) / (Write_total); 
missRateWrite       = roundf(missRateWrite * 100) / 100;                            
...
{
float missRate = (double)((getWM(cache) + getRM(cache)) * 100) / (cache->currentCycle_cache); 
missRate = roundf(missRate * 100) / 100;                            
float missRatePrefetch = (double)(( getRMPrefetch(cache)) * 100) / (cache->currentCycle_preftcher); 

...
#if SIMD_COEF_32 < 4
"@dynamic=md5($p)@fc58a609d0358176385b00970bfb2b49", 
#else
...
if ( (strstr(ct, "$pass") || strstr(ct, "$salt") || strstr(ct, "$user"))) {
...
APP_FUNC(sh,gen_s)  APP_FUNC(S,gen_s2) APP_FUNC(u,gen_u) APP_FUNC(u_lc,gen_ulc)
...
if (strstr(pScr, "$u")) goto SkipFlat; 
if (strstr(pScr, "$s2")) goto SkipFlat; 
if (strstr(pScr, "pad")) goto SkipFlat; 
if (strstr(pScr, "$c")) goto SkipFlat; 
if (!strstr(pScr, "MaxInputLen=110")) goto SkipFlat;  
while (cp2) {
...
for (j = i - 1; j >= 0; --j) {
if (!strcmp(pCode[j], "push") || (exponent >= 0 && !strcmp(pCode[j], "Xush"))) { 
last_push = j;
use_inp1_again = 0;
for (x = j+1; x < i; ++x) {
if (pCode[x][0] == 'X')
...
char *type = 0;
switch (num/10) {
case 5: type="sha224"; break; 
case 6: type="sha256"; break; 
...
strupr(cpTypeU);
sprintf(szExpr, "dynamic=%s($p)", cpType); 
ret = dynamic_compile(szExpr, &p);
if (ret) return 1;
printf("\n", cpType, Num, Num+8);
printf("DYNA_PRE_DEFINE_LARGE_HASH(%s,%s,%d)\n", cpTypeU, cpNum, (int)strlen(gen_conv)); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s($s.$p)", cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s($p.$s)", cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s(%s($p))", cpType, cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s(%s_raw($p))", cpType, cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s(%s($p).$s)", cpType, cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s($s.%s($p))", cpType, cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s(%s($s).%s($p))", cpType, cpType, cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
sprintf(szExpr, "dynamic=%s(%s($p).%s($p))", cpType, cpType, cpType); 
if (big_gen_one(Num++, szExpr)) return 1;
return 0;
}

...
#define SALT_LENGTH             (USER_NAME_LENGTH * 4)    
#define PLAINTEXT_LENGTH        40 
#define UTF8_PLAINTEXT_LENGTH   MIN(125, PLAINTEXT_LENGTH * 3) 
#define BINARY_SIZE             20
#define BINARY_ALIGN            4
#define SALT_SIZE               sizeof(struct saltstruct)
#define SALT_ALIGN              4
#define CIPHERTEXT_LENGTH       (SALT_LENGTH + 1 + 2*BINARY_SIZE)    
#ifdef SIMD_COEF_32
...
#if SIMD_COEF_32
unsigned const char *p = &pbHashArray[GETOUTSTARTPOS(index)]; 
modSum += *p++ % 6;
modSum += *p++ % 6;
modSum += *p++ % 6;
modSum += *p++ % 6;
...
unsigned char tempVar[UTF8_PLAINTEXT_LENGTH + MAGIC_ARRAY_SIZE + SALT_LENGTH]; 
SHA_CTX ctx;
if (keyLen[index] < 0) {
...
memcpy(tempVar, saved_key[index], keyLen[index]);  
memcpy(tempVar + keyLen[index], cur_salt->s, cur_salt->l); 
SHA1_Init(&ctx);

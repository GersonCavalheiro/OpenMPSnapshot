...
for (j = 0; j < inner_batch_size; ++j) {
cracked[i+j] = 0;
if (decrypt_and_verify(keys[j], 0) 
|| decrypt_and_verify(keys[j], 1) 
|| decrypt_and_verify(keys[j], 2)) 
cracked[i+j] = 1;
}
}
return count;

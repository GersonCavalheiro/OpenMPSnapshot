...
bt_uint192_t *bt_loaded_hashes_192 = NULL;
unsigned int *bt_hash_table_192 = NULL;
inline unsigned int modulo192_31b(bt_uint192_t a, unsigned int N, uint64_t shift64, uint64_t shift128)
{
uint64_t p;
p = (a.HI % N) * shift128;
p += (a.MI % N) * shift64;
...
{
unsigned int i;
if (bt_memalign_alloc((void **)&bt_hash_table_192, 32, 6 * bt_hash_table_size * sizeof(unsigned int)))
bt_error("Couldn't allocate bt_hash_table_192.");
for (i = 0; i < bt_hash_table_size; i++)
bt_hash_table_192[i] = bt_hash_table_192[i + bt_hash_table_size] = bt_hash_table_192[i + 2 * bt_hash_table_size] =
bt_hash_table_192[i + 3 * bt_hash_table_size] = bt_hash_table_192[i + 4 * bt_hash_table_size] =
bt_hash_table_192[i + 5 * bt_hash_table_size] = 0;
bt_total_memory_in_bytes += 6 * bt_hash_table_size * sizeof(unsigned int);
if (verbosity > 2) {
...
fprintf(stdout, "Hash Table Size(in GBs):%Lf\n", ((long double)6 * bt_hash_table_size * sizeof(unsigned int)) / ((long double)1024 * 1024 * 1024));
}
}
inline unsigned int calc_ht_idx_192(unsigned int hash_location, unsigned int offset)
{
return  modulo192_31b(add192(bt_loaded_hashes_192[hash_location], offset), bt_hash_table_size, shift64_ht_sz, shift128_ht_sz);
}
inline unsigned int zero_check_ht_192(unsigned int hash_table_idx)
{
return (bt_hash_table_192[hash_table_idx] || bt_hash_table_192[hash_table_idx + bt_hash_table_size] ||
bt_hash_table_192[hash_table_idx + 2 * bt_hash_table_size] || bt_hash_table_192[hash_table_idx + 3 * bt_hash_table_size] ||
bt_hash_table_192[hash_table_idx + 4 * bt_hash_table_size] || bt_hash_table_192[hash_table_idx + 5 * bt_hash_table_size]);
}
inline void assign_ht_192(unsigned int hash_table_idx, unsigned int hash_location)
{
bt_uint192_t hash = bt_loaded_hashes_192[hash_location];
bt_hash_table_192[hash_table_idx] = (unsigned int)(hash.LO & 0xffffffff);
...
{
bt_hash_table_192[hash_table_idx] = bt_hash_table_192[hash_table_idx + bt_hash_table_size] = bt_hash_table_192[hash_table_idx + 2 * bt_hash_table_size] =
bt_hash_table_192[hash_table_idx + 3 * bt_hash_table_size] = bt_hash_table_192[hash_table_idx + 4 * bt_hash_table_size] =
bt_hash_table_192[hash_table_idx + 5 * bt_hash_table_size] = 0;
}
unsigned int get_offset_192(unsigned int hash_table_idx, unsigned int hash_location)
{
unsigned int z = modulo192_31b(bt_loaded_hashes_192[hash_location], bt_hash_table_size, shift64_ht_sz, shift128_ht_sz);
...
(bt_loaded_hashes_192[p].LO == bt_loaded_hashes_192[q].LO &&	\
bt_loaded_hashes_192[p].MI == bt_loaded_hashes_192[q].MI &&	\
bt_loaded_hashes_192[p].HI == bt_loaded_hashes_192[q].HI)
#define check_non_zero(p) \
(bt_loaded_hashes_192[p].LO || bt_loaded_hashes_192[p].MI || bt_loaded_hashes_192[p].HI)
#define check_zero(p) \
(bt_loaded_hashes_192[p].LO == 0 && bt_loaded_hashes_192[p].MI == 0 && bt_loaded_hashes_192[p].HI == 0)
#define set_zero(p) \
bt_loaded_hashes_192[p].LO = bt_loaded_hashes_192[p].MI = bt_loaded_hashes_192[p].HI = 0
...
for (i = 0; i < num_loaded_hashes; i++) {
unsigned int idx = bt_loaded_hashes_192[rehash_list[i]].LO % bt_hash_table_size;
collisions[idx]++;
}
counter = 0;
for (i = 0; i < bt_hash_table_size; i++) {
...
unsigned int idx = bt_loaded_hashes_192[k].LO % bt_hash_table_size ;
if (collisions[idx] == 2) {
...
unsigned int remove_duplicates_192(unsigned int num_loaded_hashes, unsigned int bt_hash_table_size, unsigned int verbosity)
{
...
#endif
for (i = 0; i < num_loaded_hashes; i++) {
unsigned int idx = bt_loaded_hashes_192[i].LO & (bt_hash_table_size - 1);
#if _OPENMP
#pragma omp atomic
#endif
collisions[idx]++;
...
if (check_zero(i)) {
unsigned int j;
bt_loaded_hashes_192[i] = bt_loaded_hashes_192[num_unique_hashes];

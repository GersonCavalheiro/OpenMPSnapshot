...
if (mode_id != kBCMode_CBC && mode_id != kBCMode_XTS) 
goto bail;
if ((p = strtokm(NULL, "$")) == NULL) 
goto bail;
if (!isdec(p))
...
keylen = 32;
pkcs12_pbe_derive_key(2, cur_salt->iterations,  
...
keylen = 32;
pkcs12_pbe_derive_key(10, cur_salt->iterations, 
MBEDTLS_PKCS12_DERIVE_KEY,
(unsigned char*)saved_key[index],
saved_len[index], cur_salt->salt,
cur_salt->salt_size, key, keylen);

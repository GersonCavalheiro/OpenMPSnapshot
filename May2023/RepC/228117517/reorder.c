...
uint32_t v;
uint32_t radix = 4;  
uint32_t P = omp_get_max_threads();  
...
case 3  :
case 4 :
edgeList = maskGraphProcessDegree( edgeList, arguments->mmode, cache_size); 
break;
default :
edgeList = maskGraphProcessDegree( edgeList, arguments->mmode, cache_size); 
}
...
{
cache_regions[0] = edgeList->num_vertices / num_masks; 
cache_regions[1] = edgeList->num_vertices / num_masks; 
cache_regions[2] = edgeList->num_vertices / num_masks; 
}
else
{
cache_regions[0] = cache_size*8; 
cache_regions[1] = cache_regions[0]*4; 
cache_regions[2] = cache_regions[1]*4; 
}
cache_regions[3] = UINT32_MAX; 
#pragma omp parallel for
for (i = 0; i < edgeList->num_vertices; ++i)
{
...
case 3  :
case 10 :
edgeList = reorderGraphProcessDegree( arguments->sort, edgeList, arguments->lmode);
break;
case 4  :
case 5  :
edgeList = reorderGraphProcessDBG( arguments->sort, edgeList, arguments->lmode);
break;
case 6  :
case 7  :
edgeList = reorderGraphProcessHUBSort( arguments->sort, edgeList, arguments->lmode);
break;
case 8  :
case 9  :
edgeList = reorderGraphProcessHUBCluster( arguments->sort, edgeList, arguments->lmode);
break;
case 11 :
edgeList = relabelEdgeListFromFile(edgeList, arguments->fnamel, edgeList->num_vertices);
break;
case 12 :
case 13 :
edgeList = reorderGraphProcessCorder( arguments->sort, edgeList, arguments->lmode);
break;
default :
edgeList = reorderGraphProcessDegree( arguments->sort, edgeList, arguments->lmode);
}

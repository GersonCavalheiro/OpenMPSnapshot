...
#define AVX_AND_NOT(X,Y)                  _mm256_andnot_ps(X,Y)  
...
#define AVX_AND_NOT(X,Y)                  _mm256_andnot_pd((X),(Y))  
...
\
const __m128 float_tmp1 =  _mm256_cvtpd_ps(denominator); \
\
__m128 float_inv_tmp1 = _mm_rcp_ps(float_tmp1); \
const AVX_FLOATS rc = _mm256_cvtps_pd(float_inv_tmp1); \
\
\
...
AVX_MULTIPLY_FLOATS(denominator, rc_iter)));  \
}                                                           \
...
\
const AVX_FLOATS rc  = _mm256_rcp_ps(denominator); \
\
\
const AVX_FLOATS two = AVX_SET_FLOAT((DOUBLE) 2.0);         \
AVX_FLOATS rc_iter = rc;                                    \

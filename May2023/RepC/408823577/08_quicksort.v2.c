...
if ( mid_start > 0 )
#pragma omp task default(none) final( mid_start < task_cutoff ) mergeable shared(data) firstprivate(start, mid) untied
pqsort( data, start, mid );
int end_mid = end -(mid+1);
if ( end_mid )
#pragma omp task default(none) final( end_mid < task_cutoff ) mergeable shared(data) firstprivate(mid, end) untied

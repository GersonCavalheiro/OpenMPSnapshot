...
#pragma omp parallel firstprivate(vertices, n_vertices) shared(n_predecessors, n_predecessors_max)
{
...
#pragma omp target device(BIGPULP_SVM) map(to: vertices[0:n_vertices], n_vertices) map(tofrom: n_successors_max)
{
...
#pragma omp parallel firstprivate(vertices_local, n_vertices_local) shared(n_successors_max_local)
{
unsigned n_successors_tmp = 0;
...
#pragma omp target device(BIGPULP_SVM) map(to: vertices[0:n_vertices], n_vertices) map(tofrom: n_edges)
{
unsigned n_vertices_local = hero_tryread((unsigned int *)&n_vertices);
unsigned n_edges_local    = hero_tryread((unsigned int *)&n_edges);
vertex * vertices_local   = (vertex *)hero_tryread((unsigned int *)&vertices);
#pragma omp parallel firstprivate(vertices_local, n_vertices_local) shared(n_edges_local)
{
...
#pragma omp target device(BIGPULP_SVM) map(to: vertices[0:n_vertices], n_vertices) map(tofrom: n_predecessors_max, n_predecessors[0:n_vertices])
{
unsigned n_vertices_local         = hero_tryread((unsigned int *)&n_vertices);
...
#pragma omp parallel firstprivate(vertices_local, n_vertices_local, n_predecessors_local) shared(n_predecessors_max_local)
{
unsigned n_successors_tmp = 0;
unsigned vertex_id_tmp    = 0;
#pragma omp for

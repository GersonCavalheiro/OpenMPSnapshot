...
culture[i] = 0.0f;
#pragma omp parallel for if (num_cells > threshold)default(shared) schedule(guided)
for (i = 0; i < num_cells; i++)
{
cells[i].alive = true;
...
for (i = 0; i < rows * columns; i++)
culture_cells[i] = 0;
#pragma omp parallel for if (num_cells > threshold) schedule(static) reduction(max : max_age)
for (i = 0; i < num_cells; i++)
{
cells[i].age++;
...
#pragma omp parallel for if (num_cells > threshold)schedule(static)
for (i = 0; i < num_cells; i++)
{
...
#pragma omp parallel for if (num_cells > threshold) schedule(guided) reduction(+ : step_dead_cells) reduction(max : max_age)
for (i = 0; i < num_cells; i++)
{
if (cells[i].alive)
{

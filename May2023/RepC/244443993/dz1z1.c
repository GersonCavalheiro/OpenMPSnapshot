...
int start;
int end;
#pragma omp parallel default(none) private(i, j, k, juliaValue, threadID, start, end) shared(rgb, numThreads, chunkSize, w, h, xl, xr, yb, yt, cnt)
{
threadID = omp_get_thread_num();
numThreads = omp_get_num_threads();
chunkSize = (h + numThreads - 1) / numThreads;
start = threadID * chunkSize;
end = start + chunkSize < h ? start + chunkSize : h;
for (j = start; j < end; j++)
{
for (i = 0; i < w; i++)
{

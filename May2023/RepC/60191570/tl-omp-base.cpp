...
{
#define OMP_DIRECTIVE(_directive, _name, _pred) BIND_DIRECTIVE("omp", _directive, _name, _pred, )
#define OMP_CONSTRUCT(_directive, _name, _pred) BIND_CONSTRUCT("omp", _directive, _name, _pred, )
...
{
oss_loop_handler_post(directive,  true,  false);
return;
}
TL::PragmaCustomLine pragma_line = directive.get_pragma_line();
...
const_value_get_one( 0, 1),
directive.get_locus());
}
else
{
expr = const_value_to_nodecl(const_value_get_one( 0,  4));
}
}
...
Nodecl::NodeclBase code = loop_handler_post(directive, statement, barrier_at_end,  false);
pragma_line.diagnostic_unused_clauses();
directive.replace(code);
}
...
Nodecl::NodeclBase::null(), directive.get_locus()));
...
nodecl_null(), Nodecl::List::make(stmt));
}
...
return oss_loop_handler_post(directive,  false,  true);
}
...
{
return arg.make_nodecl(true, _locus);
}
Nodecl::NodeclBase operator()(DataEnvironment::DataSharingInfoPair arg) const
{
return arg.first.make_nodecl(true, _locus);
}
};
...
Nodecl::Symbol::make(arg.get_reduction()->get_symbol(), _locus),
arg.get_symbol().make_nodecl( true, _locus),
Nodecl::Type::make(arg.get_reduction_type(), _locus),
_locus);
}
};
...
ObjectList<Nodecl::NodeclBase>& result_list)
{
make_data_sharing_list<T>(data_sharing_env, data_attr,  NULL, locus, result_list);
}
...
if (IS_CXX_LANGUAGE)
{
new_body.append(Nodecl::CxxDef::make( nodecl_null(),
...
grainsize_adjustment_sym,  new_body);
}
...
get_bool_type()),
get_bool_type());
new_outer_loop = Nodecl::WhileStatement::make(condition, new_outer_loop_body,  Nodecl::NodeclBase::null());
}
else
{
...
Nodecl::Context::make(
Nodecl::List::make(
Nodecl::CompoundStatement::make(new_body,  Nodecl::NodeclBase::null())),

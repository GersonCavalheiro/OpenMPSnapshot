...
{
::fortran_load_module(module.get_name().c_str(),  0, call.get_locus());
...
instantiation_symbol_map_push( NULL);
...
{
Nodecl::Symbol symbol_ref = it->make_nodecl( true, locus);
...
if (!expr.is_null())
result_list.append(Nodecl::OmpSs::LintVerified::make(expr, locus));
}
...
in_deps, concurrent_deps, out_deps,
shared_symbols, shared_and_alloca_exprs,
firstprivate_symbols, alloca_exprs;
...
Nodecl::CxxDecl::make( nodecl_null(), new_function));
}
...
Nodecl::CxxDef::make(  nodecl_null(), sym, locus));
}
}
...
Nodecl::Utils::append_items_after(enclosing_stmt, Nodecl::OpenMP::Taskwait::make( nodecl_null()));
CXX_LANGUAGE()
{
Nodecl::Utils::prepend_items_before(enclosing_stmt,
Nodecl::CxxDef::make(  nodecl_null(), new_symbol, locus));
}
}

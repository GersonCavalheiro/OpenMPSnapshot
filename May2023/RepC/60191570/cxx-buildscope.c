...
ASTKind(a) == AST_USING_DECLARATION_TYPENAME,
nodecl_output);
break;
}
...
if (nodecl_is_err_expr(nodecl_name))
return;
introduce_using_entity_nodecl_name(nodecl_name, decl_context, current_access,  0, nodecl_output);
}
...
class_type_add_member(class_info, primary_symbol, decl_context,  1);
}
}
static void build_scope_template_alias_declaration(AST a, const decl_context_t* decl_context, nodecl_t* nodecl_output)
{
...
symbol_entity_specs_set_class_type(entry, class_info);
symbol_entity_specs_set_access(entry, access_specifier);
class_type_add_member(class_info, entry, decl_context,  1);
}
}
static void build_scope_simple_alias_declaration(AST a, const decl_context_t* decl_context, nodecl_t* nodecl_output)
{
...
{
set_is_transparent_union(entry->type_information,  1);
}
...
&& entry->decl_context->current_scope == entry->decl_context->global_scope)
|| (entry->kind == SK_FUNCTION
&& symbol_has_gcc_attribute(entry, "alias",  NULL)))))
{
*nodecl_output = nodecl_concat_lists(
*nodecl_output,
nodecl_make_list_1(
...
*type_info = get_mask_type( 8 * type_get_size(*type_info));
}
...
AST expression = advance_expression_nest_flags(ASTSon0(a),  0);
nodecl_t nodecl_expr = nodecl_null();
...
set_is_complete_type(enum_type,  1);
...
class_type_add_member(class_type, new_inherited_constructor, class_context,  1);
if (exists_constructor_with_same_characteristics(
...
class_type_add_member(class_type, implicit_default_constructor, class_context,  0);
...
class_type_add_member(class_type, implicit_copy_constructor, class_context,  1);
...
class_type_add_member(class_type, implicit_move_constructor, class_context,  1);
...
get_lvalue_reference_type(type_info), 
parameter_info,
1, REF_QUALIFIER_NONE);
...
class_type_add_member(class_type, implicit_copy_assignment_function, class_context,  1);
...
class_type_add_member(class_type, implicit_move_assignment_function, class_context,  1);
...
class_type_add_member(class_type, implicit_destructor, class_context,  1);
...
{
set_is_transparent_union(class_type,  1);
}
}
...
scope_entry_t* enclosing_class_symbol = decl_context->current_scope->related_entry;
type_t* enclosing_class_type = enclosing_class_symbol->type_information;
class_type_add_member(enclosing_class_type, class_entry, decl_context,  1);
CXX_LANGUAGE()
{
symbol_entity_specs_set_is_member(class_entry, 1);
...
set_is_complete_type(class_type,  1);
set_is_complete_type(get_actual_class_type(class_type),  1);
...
C_LANGUAGE()
{
*function_type = get_nonproto_function_type(*function_type, 0);
}
CXX_LANGUAGE()
{
*function_type = get_new_function_type(*function_type, parameter_info, 0, ref_qualifier);
}
return;
}
AST iter = NULL;
...
entity_context, prototype_context,  NULL, nodecl_output);
if (is_error_type(*declarator_type))
{
return;
}
...
set_parameters_as_related_symbols(entry, gather_info,  0,
ast_get_locus(declarator_id));
}
{
...
if ((!is_template_function && is_template_id) 
|| (is_template_function && is_qualified)) 
{
...
linkage_push(current_linkage,  1);
...
if (strcmp(current_linkage, "\"C++\"") == 0)
current_linkage = NULL;
linkage_push(current_linkage,  0);
...
nodecl_t* nodecl_output)
{
push_new_template_header_level(decl_context, template_context,  0);
...
{
decl_context_t* template_context;
push_new_template_header_level(decl_context, &template_context,  1);
...
gather_decl_spec_list_t* gather_decl_spec_list)
{
build_scope_function_definition(
function_definition,
decl_context,
1, is_explicit_specialization, nodecl_output, 
declared_symbols, gather_decl_spec_list);
}
static void build_scope_template_deleted_function_definition(
AST function_declaration,
...
0, template_context, nodecl_output);
break;
...
build_scope_type_template_parameter(a, template_parameter_list, nesting,
1, template_context, nodecl_output);
break;
...
1, template_params_context, nodecl_output);
...
primary_type, template_parameter_name, template_context,
new_entry->locus);
...
symbol_set_as_parameter_of_function(current_param, entry,  0,  i);
}
if (is_definition
...
{
check_constexpr_function(entry, nodecl_get_locus(body_nodecl),  1,  1);
}
check_constexpr_function_body(entry, body_nodecl,  1,  1);
}
if (is_dependent_function(entry))
{
...
{
build_scope_common_template_alias_declaration(a, decl_context, nodecl_output,
1, class_info, current_access, is_explicit_specialization);
}
static void build_scope_default_or_delete_template_member_function_definition(
const decl_context_t* decl_context, 
AST a, access_specifier_t current_access, type_t* class_info,
...
{
class_type_add_member(get_actual_class_type(class_info), entry, decl_context,  1);
}
...
class_type_add_member(get_actual_class_type(class_type), bitfield_symbol, decl_context,  1);
if (current_gather_info.is_static)
{
...
prettyprint_in_buffer(expression));
nodecl_bit_size = const_value_to_nodecl(const_value_get_one(  4,  1));
}
...
class_type_add_member(class_type, new_member, decl_context,  1);
}
...
AST synthesized_loop_name = ASTSon3(a); 
...
nodecl_t node)
{
instantiate_cxx_def_or_decl(v, node, nodecl_make_cxx_decl,  0);
}
static void instantiate_cxx_def(
nodecl_instantiate_stmt_visitor_t* v,
nodecl_t node)
{
instantiate_cxx_def_or_decl(v, node, nodecl_make_cxx_def,  1);
}

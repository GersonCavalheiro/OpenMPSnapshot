...
#include <string.h>
#include <math.h>
void __attribute((noinline)) cholesky_in_place(int N, float a[N][N])
{
int j;
for (j = 0; j < N; j++)
{
...
int i2;
float a_jj = a[j][j];
#pragma omp simd reduction(-:a_jj) aligned(a:64) suitable(N)
for (i2 = 0; i2 < j; i2++)
{
float tmp = a[j][i2];
a_jj -= tmp * tmp;
...
a[j][j] = a_jj;
int i3;
#pragma omp simd aligned(a:64) suitable(N)
for (i3 = j + 1; i3 < N; i3++)
{
a[i3][j] = a[i3][j] / a_jj;
}

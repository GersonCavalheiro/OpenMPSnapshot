...
|| sigaltstack(&alternate_stack,  NULL) != 0)
{
...
int result = 0;
result |= sigaction(SIGSEGV, &terminating_sigaction,  NULL);
result |= sigaction(SIGQUIT, &terminating_sigaction,  NULL);
result |= sigaction(SIGINT,  &terminating_sigaction,  NULL);
result |= sigaction(SIGTERM, &terminating_sigaction,  NULL);
result |= sigaction(SIGABRT, &terminating_sigaction,  NULL);
if (result != 0)
{
...
{
if ((argument[2] == '\0') 
|| ((argument[2] == 'P') && (argument[3] == '\0')) 
|| ((argument[2] == 'D') && (argument[3] == '\0')) 
|| ((argument[2] == 'M') && (argument[3] == '\0')) 
|| ((argument[2] == 'M') && (argument[3] == 'D') && (argument[4] == '\0'))) 
{
...
else if (((argument[2] == 'F') && (argument[3] == '\0')) 
|| ((argument[2] == 'G') && (argument[3] == '\0')) 
|| ((argument[2] == 'T') && (argument[3] == '\0'))) 
{
if (!dry_run)
{
add_to_parameter_list_str(&CURRENT_CONFIGURATION->preprocessor_options, argument);
...
merge_sort_list_str(list_config_files, num_config_files,  1);
int i;
...
continue;
if (configuration ==
get_sublanguage_configuration(SOURCE_SUBLANGUAGE_CUDA,  NULL))
continue;
...
if (std_flag != NULL)
{
add_parameter_all_toolchain_configuration(std_flag,  false, configuration);
}
}
}
static void commit_configuration(void)
...
{
config_add_preprocessor_prefix(configuration,  NULL, "omp");
}
else
{
...
static void register_upc_pragmae(compilation_configuration_t* configuration)
{
config_add_preprocessor_prefix(configuration,  NULL, "upc");
register_new_directive(configuration, "upc", "relaxed",  0,  0);
register_new_directive(configuration, "upc", "strict",  0,  0);
config_add_preprocessor_prefix(configuration,  NULL, "distributed");
register_new_directive(configuration, "distributed", "",  0,  0);
}
...
if (!file_not_processed)
{
native_compilation(translation_unit, prettyprinted_filename,  1);
}
else
{
native_compilation(translation_unit, translation_unit->input_filename,  0);
}
}
if (current_extension->source_language == SOURCE_LANGUAGE_FORTRAN
&& !CURRENT_CONFIGURATION->do_not_compile)
...
preprocessor_options, stdout_file,  NULL);

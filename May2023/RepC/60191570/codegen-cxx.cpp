...
state.in_condition = 1;
define_or_declare_variable(node.get_symbol(),  1);
...
template_args.get_num_parameters(),
0,
...
{
visit_function_call(node,  false);
}
...
{
declarator_name = symbol.get_class_qualification(function_scope,  true);
}
else
{
...
if (!symbol.is_defined_inside_class())
{
declarator_name = symbol.get_class_qualification(symbol_scope,  true);
}
else
{
declarator_name = unmangle_symbol_name(symbol);
...
bool old_in_condition = state.in_condition;
state.in_condition = 1;
define_or_declare_variable(iterator_symbol,  1);
state.in_condition = old_in_condition;
*file << " : ";
walk(node.get_initializer());
}
...
TYPECAST, 
COMPOUND_LITERAL, 
EXPLICIT_TYPECAST_PARENTHESIZED, 
EXPLICIT_TYPECAST_BRACED, 
IMPLICIT_BRACED, 
UNKNOWN, 
...
state.must_be_object_init.erase(sym);
move_to_namespace_of_symbol(sym);
codegen_explicit_instantiation(sym, declarator_name, context,  true);
}
...
{
codegen_template_headers_all_levels(template_parameters,  true);
}
}
else
{
...
if (level == 0)
{
qualified_name = symbol.get_class_qualification( true);
}
else
{
...
{
*(file) << this->get_qualified_name(entry,  1) << ";\n";
}
}
...
define_class_symbol_aux(symbol, symbols_defined_inside_class,  0, scope);
state.classes_being_defined.pop_back();
...
{
define_or_declare_variable(symbol,  true);
}
...
move_to_namespace_of_symbol(symbol);
TL::TemplateParameters template_parameters = symbol.get_scope().get_template_parameters();
codegen_template_header(template_parameters,  true);
indent();
...
if (symbol.is_variable())
{
define_or_declare_variable(symbol,  false);
return;
}
...
{
function_name = symbol.get_class_qualification(symbol.get_scope(),  true);
}
}
if (symbol.get_type().is_template_specialized_type()
&& !is_primary_template
...
if (is_pointer_arithmetic_add(add,  pointer_type))
{
walk_type_for_symbols(
...
push_scope(symbol.get_scope());
this->set_last_is_newline(false); 
...
name.c_str(), "", 0, 0, NULL, NULL,  0,  0,
print_name_str,  (void*) this);
}

...
reduction_type,
const_value_get_zero( type_get_size(reduction_type),  1),
make_locus("", 0, 0)));
}
...
{
compute_nodecl_udr_(tree, decl_context, nodecl_output,  true);
}
static void compute_nodecl_udr_regular(AST tree, const decl_context_t* decl_context, nodecl_t* nodecl_output)
{
compute_nodecl_udr_(tree, decl_context, nodecl_output,  false);
}
...
parse_declare_reduction(directive.get_context_of_decl(), declare_reduction_src,  false);
}
...
return lookup(sc, id_expression_orig, t,  false, allow_array_types);
}

...
_env.reduction.insert(ReductionItem(reduction_symbol, reduction_type, red,  false));
}
}
...
OpenMP::Reduction* red = OpenMP::Reduction::get_reduction_info_from_symbol(reductor_sym);
ERROR_CONDITION(red == NULL, "Invalid value for red_item", 0);
_env.in_reduction.insert(ReductionItem(reduction_symbol, reduction_type, red,  false));
}
}
...
OpenMP::Reduction* red = OpenMP::Reduction::get_reduction_info_from_symbol(reductor_sym);
ERROR_CONDITION(red == NULL, "Invalid value for red_item", 0);
_env.reduction.insert(ReductionItem(reduction_symbol, reduction_type, red,  true));
}
}
template < typename T >
...
constrains["node"].default_value = node_default.make_nodecl( true);

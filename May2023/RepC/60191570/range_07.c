...
#pragma omp task depend (in: p[0:BS], saturationLimit) depend (inout: saturationFrame[i][j])
...
for (j=0; j < DIM_X; j++) {
p_block_type p = (*currentFrame[groupNumber-1])[i][j];
#pragma omp task depend (in: biasFrame[i][j]) depend (inout: p[0:BS])
...
for (j=0; j < DIM_X; j++) {
p_block_type p = (*currentFrame[groupNumber-1])[i][j];
#pragma omp task depend (in: coeffOfNonLinearityPolynomial) depend (inout: p[0:BS])
...
for (j=0; j < DIM_X; j++) {
p_block_type p = (*currentFrame[groupNumber-1])[i][j];
#pragma omp task firstprivate(groupNumber) depend (in: p[0:BS], sumXYFrame[i][j], sumYFrame[i][j]) depend (inout: offsetCosmicFrame[i][j], numberOfFramesAfterCosmicRay[i][j])
...
for (j=0; j < DIM_X; j++) {
p_block_type p = (*currentFrame[groupNumber-1])[i][j];
#pragma omp task firstprivate(groupNumber) depend (in: p[0:BS], offsetCosmicFrame[i][j], saturationFrame[i][j]) depend (inout: sumXYFrame[i][j], sumYFrame[i][j])
...
#pragma analysis_check assert range(i:0:3:0; j:0:4:0)
for (j=0; j < DIM_X; j++) {
#pragma omp task depend (in: sumXYFrame[i][j]) depend (inout: sumYFrame[i][j])

...
if (it != map.m.end())
{
return it->second.make_nodecl( true);
}
}
Nodecl::NodeclBase::Children children = node.children();
for (Nodecl::NodeclBase::Children::iterator it = children.begin();
...
Nodecl::Symbol sym_ref = new_symbol.make_nodecl( true);
...
int parameter_position_offset = 0; 
...
Nodecl::NodeclBase sym_ref = new_symbol.make_nodecl( true);
...
Nodecl::NodeclBase new_construct = Nodecl::OpenMP::Task::make( Nodecl::NodeclBase::null(), statements, statements.get_locus());
...
0,
symbol_entity_specs_get_num_related_symbols(new_function_sym));
...
called_function,  1,  _function_task_set);
if (!_lowering->final_clause_transformation_disabled()
&& Nanos::Version::interface_is_at_least("master", 5024)
...
OutlineInfo new_outline_info(*_lowering, new_environment, called_task_function,  1,  _function_task_set);
TaskEnvironmentVisitor task_environment;
task_environment.walk(new_environment);
...
new_function_call.set_called(adapter_function.make_nodecl( true));
...
it++)
{
arguments.append(it->make_nodecl( true));
}

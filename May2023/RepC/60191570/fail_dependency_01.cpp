...
const objT * __restrict__ const y0, 
...
rsize = MAX(j + bsNA - (jl + iOne)*lda, iZero) % lda, 
r1    = MAX(rsize, iOne)                            ;
...
#pragma oss task default(none) firstprivate(layout, trans, alpha, beta, incx, incy)         firstprivate(j, jrem, m, bsNA, lda) firstprivate(x, p_al, y) in (         x             [iZero;m    ])                    in (([m][lda]p_al)[iZero;m][jrem ;lsize])                    out(         y             [j    ;bsNA ])
{
}
}
}

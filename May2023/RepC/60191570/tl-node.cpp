...
NodeclSet Node::get_lastprivate_vars()
{
NodeclSet lastprivate_vars;
const TL::Analysis::PCFGPragmaInfo& task_pragma_info = get_pragma_node_info();
if (task_pragma_info.has_clause(NODECL_OPEN_M_P_LASTPRIVATE))
{
Nodecl::List tmp = task_pragma_info.get_clause(NODECL_OPEN_M_P_LASTPRIVATE).as<Nodecl::OpenMP::Lastprivate>().get_symbols().shallow_copy().as<Nodecl::List>();
lastprivate_vars.insert(tmp.begin(), tmp.end());
}
return lastprivate_vars;
}
...
if (task_pragma_info.has_clause(NODECL_OMP_SS_DEP_CONCURRENT))
{
Nodecl::List tmp = task_pragma_info.get_clause(NODECL_OMP_SS_DEP_CONCURRENT).as<Nodecl::OmpSs::DepConcurrent>().get_exprs().shallow_copy().as<Nodecl::List>();
shared_vars.insert(tmp.begin(), tmp.end());
}
if (task_pragma_info.has_clause(NODECL_OMP_SS_DEP_COMMUTATIVE))
{
Nodecl::List tmp = task_pragma_info.get_clause(NODECL_OMP_SS_DEP_COMMUTATIVE).as<Nodecl::OmpSs::DepCommutative>().get_exprs().shallow_copy().as<Nodecl::List>();
shared_vars.insert(tmp.begin(), tmp.end());
}
...
if (Nodecl::Utils::structurally_equal_nodecls((*it)->get_variable(), iv, true))
return true;
return false;
}
...
if (it != ranges.end())
{   
if (Nodecl::Utils::structurally_equal_nodecls(it->second, value, true))
return;

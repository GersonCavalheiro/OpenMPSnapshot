...
new_target_info._target_symbol = _target_symbol; 
new_target_info._device_list = _device_list;
new_target_info._implementation_table = _implementation_table; 
...
instantiate_expression(data_ref.get_internal_nodecl(), instantiation_context, instantiation_symbol_map,  -1);
DataReference updated_data_ref(updated_expr);
...
{
Nodecl::NodeclBase updated_expr =
instantiate_expression(it->get_internal_nodecl(), instantiation_context, instantiation_symbol_map, -1);
...
instantiation_context, instantiation_symbol_map,  -1);

...
static void make_empty_csc(temp_csc_graph* restrict const outg ) {
...
static void make_csc(const packed_edge* restrict const inbuf, temp_csc_graph* restrict const outg ) {
...
temp_csc_graph t = { \
(size_t*)xmalloc((t_nrows + 1) * sizeof(size_t)), \
(int64_t*)xmalloc((size_t)(EDGES_RECEIVED_THIS_BLOCK) * sizeof(int64_t)), \
(size_t)(nlocalverts_so_far), \
(int)(LG_NGLOBALVERTS_SO_FAR), \
(int64_t)(INT64_C(1) << (LG_NGLOBALVERTS_SO_FAR)), \
(size_t)(EDGES_RECEIVED_THIS_BLOCK), \
(size_t)(EDGES_RECEIVED_THIS_BLOCK), \
-1,  \
t_nrows \
}; \

...
int64_t* restrict pred_pred = (int64_t*)xMPI_Alloc_mem(size_min(CHUNKSIZE, nlocalverts) * sizeof(int64_t)); 
...
int64_t* restrict pred_vtx = (int64_t*)xmalloc(size_min(CHUNKSIZE, nlocalverts) * sizeof(int64_t)); 
...
for (i = i_start; i < i_end; ++i) {
if (rank == root_owner && (size_t)i == root_local) continue;
if (get_pred_from_pred_entry(pred[i]) == -1) continue; 
if (get_depth_from_pred_entry(pred_pred[i - i_start]) == UINT16_MAX) {
...
if (root_owner == rank && root_local == (size_t)i) found_pred_edge = 1; 
if (!found_pred_edge) {

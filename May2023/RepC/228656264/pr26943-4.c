...
omp_set_dynamic (0);
omp_set_nested (1);
#pragma omp parallel num_threads (2) reduction (+:l) firstprivate (a, b, c, d, e, f, g, h, j)
if (k == omp_get_thread_num ())
{
#pragma omp parallel for shared (a, e) firstprivate (b, f) lastprivate (c, g) private (d, h) schedule (static, 1) num_threads (4) reduction (+:j)
for (i = 0; i < 4; i++)
{
